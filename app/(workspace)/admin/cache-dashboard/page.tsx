...
import {
  getCacheMetrics as getServerCacheMetrics,
  monitorCacheHealth,
...
async function getCacheStats() {
  ...
  const workspaceStats = await withWorkspace(async (ctx) =>
    getWorkspaceCacheStats(ctx.workspaceId)
...
export default async function CacheDashboardPage() {
  ...
  const hasAccess = await withWorkspace(async (ctx) =>
    ctx.hasPermission('workspace:settings')
  ).catch(() => false)
  ...
  const { serverMetrics, redisMetrics, workspaceStats, health } =
    await getCacheStats()

  // Calculate overall metrics
  const overallHitRate =
    ((redisMetrics.hits + serverMetrics.hits) /
      (redisMetrics.total + serverMetrics.totalQueries)) *
      100 || 0
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <MetricCard
          ...
          target=">85%"
          status={
            parseFloat(redisMetrics.hitRate) >= 85
              ? 'good'
              : parseFloat(redisMetrics.hitRate) >= 70
                ? 'warning'
                : 'bad'
          }
        />
        ...
        <MetricCard
          ...
          target=">90%"
          status={
            serverMetrics.hitRate >= 90
              ? 'good'
              : serverMetrics.hitRate >= 70
                ? 'warning'
                : 'bad'
          }
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MetricCard
          ...
          target="<100ms"
          status={
            serverMetrics.averageLatency < 100
              ? 'good'
              : serverMetrics.averageLatency < 200
                ? 'warning'
                : 'bad'
          }
        />
        ...
        <MetricCard
          ...
          status={
            serverMetrics.slowQueries === 0
              ? 'good'
              : serverMetrics.slowQueries < 10
                ? 'warning'
                : 'bad'
          }
        />
        ...
        <MetricCard
          ...
          status={
            serverMetrics.errors === 0
              ? 'good'
              : serverMetrics.errors / serverMetrics.totalQueries < 0.05
                ? 'warning'
                : 'bad'
          }
        />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Redis memory consumption by cache type
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-semibold">
                  {(workspaceStats.memoryUsage / 1024 / 1024).toFixed(2)} MB /
                  150 MB
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              {Object.entries(workspaceStats.keysByType).map(
                ([type, count]) => (
                  <div key={type}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Document and workspace cache statistics
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              ...
              <StatRow
                ...
                value={workspaceStats.totalKeys}
              />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <StatRow
                label="Total Queries"
                value={serverMetrics.totalQueries}
              />
