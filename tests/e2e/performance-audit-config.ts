...
export const PERFORMANCE_AUDIT_CONFIG = {
  ...
  selectors: {
    // Authentication
    loginButton:
      'a[href*="login"], button:has-text("Logga in"), button:has-text("Sign in")',
    emailInput:
      'input[type="email"], input[name="email"], input[placeholder*="mail"]',
    passwordInput: 'input[type="password"], input[name="password"]',
    submitButton:
      'button[type="submit"], button:has-text("Logga in"), button:has-text("Sign in")',

    // Workspace
    workspaceSwitcher:
      '[data-testid="workspace-switcher"], button:has-text("workspace"), .workspace-selector',
...
const preset = process.env
  .PERFORMANCE_PRESET as keyof typeof PERFORMANCE_PRESETS
if (preset && PERFORMANCE_PRESETS[preset]) {
  Object.assign(
    PERFORMANCE_AUDIT_CONFIG.performanceTargets,
