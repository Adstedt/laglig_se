...
async function authenticate(page: Page) {
  ...
  const workspaceSelector = page.locator(
    '[data-testid="workspace-switcher"], .workspace-selector, button:has-text("workspace")'
  )
  ...
  const passwordInput = page
    .locator(TEST_CONFIG.selectors.passwordInput)
    .first()
...
test.describe('Laglig.se Performance Audit', () => {
  ...
  test('1. Workspace Selector Performance', async ({ page }) => {
    ...
    const workspaceSelector = page
      ...
      .or(
        page
          .locator('button:has-text("workspace")')
          .or(page.locator('.workspace-selector'))
      )
      .first()
    ...
    const dropdown = page
      .locator('[role="menu"], .dropdown-menu, .workspace-dropdown')
      .first()
    ...
    const workspaceItems = page
      .locator('[role="menuitem"], .workspace-item')
      .or(dropdown.locator('button, a').filter({ hasText: /\w/ }))
    ...
    if (workspaceCount > 1) {
      ...
      for (let i = 0; i < Math.min(3, workspaceCount); i++) {
        ...
        const workspaceName =
          (await workspace.textContent()) || `Workspace ${i}`
        ...
        console.log(
          `Workspace switch time for ${workspaceName}: ${switchTime}ms`
        )
        await takeScreenshot(page, `workspace-${i}-loaded`)

        // Validate performance target
        expect(switchTime).toBeLessThan(
          TEST_CONFIG.performanceTargets.workspaceSwitch
        )
  ...
  test('2. Law Lists (Laglistor) Navigation Performance', async ({ page }) => {
    ...
    const sidebar = page
      .locator('.sidebar, [data-testid="sidebar"], nav')
      .first()
    ...
    const listItems = page
      .locator('.law-list-item, .list-item, tr, .card')
      .filter({ hasText: /\w/ })
    ...
    if (itemCount > 0) {
      ...
      if (await table.isVisible().catch(() => false)) {
        ...
        const sortButton = page
          .locator('th button, .sort-header, [aria-sort]')
          .first()
        ...
        const nextButton = page
          .locator(
            'button:has-text("Next"), button:has-text("Nästa"), .pagination button'
          )
          .last()
  ...
  test('3. Law List Item Modal Performance', async ({ page }) => {
    ...
    const lawItems = page
      .locator('.law-item, .document-item, tr td:first-child, .card-title')
      .filter({ hasText: /\w/ })
    ...
    if (itemCount > 0) {
      ...
      expect(firstOpenTime).toBeLessThan(
        TEST_CONFIG.performanceTargets.modalFirstOpen
      )

      // Close modal
      const closeButton = page
        .locator(
          'button[aria-label="Close"], button:has-text("×"), .modal-close'
        )
        .first()
      ...
      expect(cachedOpenTime).toBeLessThan(
        TEST_CONFIG.performanceTargets.modalCached
      )
  ...
  test('4. Settings (Inställningar) Page Performance', async ({ page }) => {
    ...
    // Find settings tabs
    const tabs = page
      .locator('[role="tab"], .tab, .settings-tab')
      .filter({ hasText: /\w/ })
    ...
    if (tabCount > 1) {
      ...
      for (let i = 0; i < Math.min(5, tabCount); i++) {
        ...
        expect(tabLoadTime).toBeLessThan(
          TEST_CONFIG.performanceTargets.listLoads
        )
      }
    }
  ...
  test('5. Legal Sources (Rättskällor) Performance', async ({ page }) => {
    ...
    // Browse results
    const results = page
      .locator('.result-item, .search-result, .card, article')
      .filter({ hasText: /\w/ })
    ...
    if (resultCount > 0) {
      ...
      for (let i = 0; i < Math.min(3, resultCount); i++) {
        ...
        expect(openTime).toBeLessThan(
          TEST_CONFIG.performanceTargets.pageNavigations
        )
        ...
        if (i < Math.min(2, resultCount - 1)) {
          ...
          console.log(
            `Legal source ${i} forward navigation time: ${forwardTime}ms`
          )
    ...
    const searchInput = page
      .locator(
        'input[type="search"], input[placeholder*="search"], input[placeholder*="sök"]'
      )
      .first()
  ...
  test('6. Tasks (Uppgifter) Page Performance and UI Freeze Detection', async ({
    page,
  }) => {
    ...
    const _freezeStartTime = Date.now()
    ...
    expect(navTime).toBeLessThan(TEST_CONFIG.performanceTargets.pageNavigations)
    expect(maxFreezeTime).toBeLessThan(
      TEST_CONFIG.performanceTargets.maxFreezeTime * 2
    ) // Allow some tolerance

    // Test task interactions
    const taskItems = page
      .locator('.task-item, .card, tr')
      .filter({ hasText: /\w/ })
    ...
    const createTaskButton = page
      .locator(
        'button:has-text("Create"), button:has-text("Skapa"), .create-task, [aria-label*="create"]'
      )
      .first()
    ...
    if (await createTaskButton.isVisible().catch(() => false)) {
      ...
      expect(modalOpenTime).toBeLessThan(
        TEST_CONFIG.performanceTargets.modalFirstOpen
      )
  ...
  test('7. Generate Performance Report', async ({ page }) => {
    ...
    // Analyze results
    const responseTimeSum = Object.values(allMeasurements).reduce(
      (sum, time) => sum + time,
      0
    )
    report.summary.averageResponseTime =
      responseTimeSum / Object.keys(allMeasurements).length

    // Check against targets
    Object.entries(allMeasurements).forEach(([key, value]) => {
      let target = TEST_CONFIG.performanceTargets.pageNavigations // default

      if (key.includes('workspace-switch'))
        target = TEST_CONFIG.performanceTargets.workspaceSwitch
      else if (key.includes('modal') && key.includes('first'))
        target = TEST_CONFIG.performanceTargets.modalFirstOpen
      else if (key.includes('modal') && key.includes('cached'))
        target = TEST_CONFIG.performanceTargets.modalCached
      else if (key.includes('list') || key.includes('tab'))
        target = TEST_CONFIG.performanceTargets.listLoads
    ...
    expect(report.summary.failedTargets).toBeLessThan(
      report.summary.totalTests * 0.3
