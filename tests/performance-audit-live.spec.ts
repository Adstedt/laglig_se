...
test.describe('Laglig.se Performance Audit - LIVE', () => {
  ...
  test('Complete Performance Audit - Live Site', async ({ page }) => {
    ...
    const workspaceSwitcher = page.locator('[data-testid="workspace-switcher"], button:has-text("Workspace"), button:has-text("Arbetsplats")').first()
    
    if (await workspaceSwitcher.isVisible()) {
      const workspaceSwitcherStart = Date.now()
      await workspaceSwitcher.click()
      await page.waitForSelector('[role="menuitem"], [data-radix-collection-item]', { timeout: 5000 })
      ...
      const workspaces = await page.locator('[role="menuitem"], [data-radix-collection-item]').all()
      console.log(`  Found ${workspaces.length} workspaces`)
      
      // Try to switch to Grro Technologies
      const grroWorkspace = page.locator('[role="menuitem"]:has-text("Grro Technologies"), [data-radix-collection-item]:has-text("Grro Technologies")').first()
    ...
    const laglistorButton = page.locator('button:has-text("Laglistor"), div:has-text("Laglistor")').first()
    ...
    if (await minaLaglistor.isVisible()) {
      ...
      await page.screenshot({ path: 'screenshots/03-mina-laglistor.png', fullPage: true })
      
      // Look for law list items
      await page.waitForTimeout(2000) // Let content load
      const lawListItems = await page.locator('tr[data-testid="law-list-item"], div[data-testid="law-list-item"], button:has-text("SFS"), td:has-text("SFS")').count()
      ...
      if (lawListItems > 0) {
        ...
        const lawItem = page.locator('button:has-text("SFS"), td:has-text("SFS"), a:has-text("SFS")').first()
        
        if (await lawItem.isVisible()) {
          // First modal open
          await tracker.measureNavigation(page, '4.1 First Modal Open (Critical Test)', async () => {
            await lawItem.click()
            await page.waitForSelector('[role="dialog"], [data-radix-dialog-content]', { timeout: 15000 })
          })
          
          await page.screenshot({ path: 'screenshots/04-modal-first-open.png', fullPage: true })
    ...
    const settingsLink = page.locator('a:has-text("Inställningar"), nav a[href*="settings"]').first()
    if (await settingsLink.isVisible()) {
      ...
      await page.screenshot({ path: 'screenshots/05-settings-page.png', fullPage: true })
    }
    ...
    if (await rattskallor.isVisible()) {
      ...
      if (await browseLink.isVisible()) {
        ...
        await page.screenshot({ path: 'screenshots/06-browse-all.png', fullPage: true })
      }
    }
    ...
    const tasksLink = page.locator('a:has-text("Uppgifter"), nav a[href*="tasks"]').first()
    if (await tasksLink.isVisible()) {
      await tracker.measureNavigation(page, '7.1 Navigate to Tasks (CRITICAL)', async () => {
      ...
      await page.screenshot({ path: 'screenshots/07-tasks-page.png', fullPage: true })
    ...
    if (await dashboardLink.isVisible()) {
      ...
      await page.screenshot({ path: 'screenshots/08-dashboard-return.png', fullPage: true })
    }
    ...
    report.slice(0, 5).forEach(metric => {
      const status = metric.totalDuration > 3000 ? '🔴' : metric.totalDuration > 1000 ? '🟡' : '🟢'
    ...
    const warnings = report.filter(m => m.totalDuration > 2000 && m.totalDuration <= 5000)
    
    console.log(`\n⚠️ Critical Issues (>5s): ${criticalIssues.length}`)
    criticalIssues.forEach(m => console.log(`   - ${m.name}: ${m.totalDuration}ms`))
    ...
    console.log(`  Actual: ${report.filter(m => m.totalDuration < 1000).length}/${report.length} operations under 1s`)
  })
})