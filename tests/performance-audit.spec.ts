...
test.describe('Laglig.se Performance Audit', () => {
  ...
  test('Complete Performance Audit', async ({ page }) => {
    ...
    const grroWorkspace = await page
      .locator('[role="menuitem"]:has-text("Grro Technologies")')
      .first()
    ...
    const lawListItems = await page
      .locator('[data-testid="law-list-item"]')
      .count()
    ...
    const warnings = report.filter(
      (m) => m.totalDuration > 1000 && m.totalDuration <= 2000
    )
