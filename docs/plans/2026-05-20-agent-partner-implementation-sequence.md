...
- **WDS-001** — **no workspace-document (styrdokument) discovery tool.** Surfaced 2026-05-28 during the 14.30 smoke (`docs/qa/gates/14.30-transition-document-status-approval.yml`). When the owner asked the agent to "Skicka arbetsmiljö utkastet till granskning", the agent correctly reached for `search_workspace_files` — but **that tool is `USER_FILE`-only per Story 17.9c**. The WORKSPACE_DOCUMENT chunks shipped in 17.9b have no consumer tool. The agent honestly refused to guess at an ID and asked for clarification. **Not a 14.30 defect** — exposes a real coverage gap. 14.30 is the first story that needs the agent to *find* a styrdokument before acting on it (14.24 *creates*, doesn't need to find). **Fix options:** (a) extend `search_workspace_files` with a `documentType: 'WORKSPACE_DOCUMENT'` switch, or (b) ship a separate `search_workspace_documents` tool over the 17.9b chunks. Symmetric to SLI-001's name-matching gap. Likely a small follow-up story.
