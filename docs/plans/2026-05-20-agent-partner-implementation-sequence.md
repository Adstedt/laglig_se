...
15. **19.7 — SPLIT 2026-05-26** (it ballooned after the 19.6 re-scope dumped the integration in; oversized as one story):
    ...
    - **19.7c** per-skill tool-registry narrowing (`createAgentTools` `activeSkills` + `ALWAYS_AVAILABLE ∪ whitelists` + route wiring) — **deferred by PO 2026-05-26**: regression-prone + not urgent (PRD Risk 4 triggers at ~30 tools; currently 27). Do it once ≥2 skills exist (post-19.7b) + a no-regression harness confirms no change-context tool is lost. Skills already declare `tools` frontmatter (forward-compat). Deps: 19.7a + 19.7b.
