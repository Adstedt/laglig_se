...
- **TC-001** — **tool-chip error state.** Surfaced 2026-05-28 during the 14.30 smoke (specifically Smoke D, the invalid-ladder probe). When a write tool *errors* (e.g. returns a `wrapToolError` envelope), the collapsed tool-call chip in `chat-message.tsx` falls back to `TOOL_CONFIG.doneLabel` — which for write tools reads as past-tense success (e.g. `"Ändrade status"`). The `proposalLabel` only fires when the response carries a `pendingActionId`; there's no "error" branch in the chip render. **NOT 14.30-specific** — every write tool in the registry has this same fallback. Cosmetic but misleading (implies the action succeeded when it actually errored). **Fix:** add an `errorLabel?` to `TOOL_CONFIG` and a third branch in the chip render that fires when the response is a `ToolError`. Cross-cutting follow-up; touches `components/features/ai-chat/chat-message.tsx`.
