...
**➡️ Next: Phase 4 — Skills.** **19.6** (skill loader + directory convention + context-bound activation) → **19.7** (ship `assess_change` migrated from the `ASSESSMENT_WORKFLOW` literal + the new `gap_analysis` skill — which orchestrates the four 19.3 diagnostic tools just shipped). Siblings **14.29** (`add_task_comment`) + **14.30** (`transition_document_status`) batch into Phase 4; **14.31** (staleness) retrofits the approval types (14.28 already populates `entity_version` for it).
