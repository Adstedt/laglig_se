...
**Delivers:**
- **Template architecture (Story 26.1)** — three layout primitives (Feature template / Industry template / Topic template) composed from ~8 reusable section components (`<MarketingHero>`, `<FeatureGrid>`, `<ProofBlock>`, `<CtaBlock>`, `<CatalogLawList>`, `<ChangeFeedEmbed>`, `<FaqAccordion>`, `<RelatedPagesGrid>`). All three layouts share the same `<MarketingShell>` (navbar + footer + breadcrumbs) and the same metadata generator (title/description/canonical/OG image/JSON-LD bundled into a single `generateMarketingMetadata()` helper). Content surface is MDX-in-repo (`content/marketing/{funktioner,branscher,omraden}/*.mdx`), frontmatter-driven, with a strict TypeScript-typed schema per template kind so missing fields fail at build time.
...
**Dependencies:**
- **Sitemap chunking** (`feat(seo): chunked sitemap + sitemap index`, merged 2026-05-16): provides `/sitemap-index.xml` and chunked `/sitemap/<n>.xml`. New marketing pages must register into the sitemap via the existing `app/sitemap.ts` `MetadataRoute.Sitemap` return shape; the page-template architecture in 26.1 includes a registration helper that walks the MDX folder and emits sitemap entries automatically (no per-page manual addition).
- **GA4 + Consent Mode v2** (`feat(consent)`, merged 2026-05-15): CTA telemetry rides the existing `gtag` channel; consent-gated as analytics is. UTM tagging in 26.1 is the additional layer.
...
- **Brand chrome** (`components/shared/navigation/{navbar,footer}.tsx`, `app/globals.css` font + color tokens): existing brand-shell wraps every marketing page. 26.2 expands the navbar; 26.1 reuses the existing footer.
...
- Reference: `app/globals.css` (Safiro + Google Sans Flex fonts, warm off-white palette, sage secondary)
- Reference: `app/sitemap.ts`, `app/sitemap-index.xml/route.ts`, `lib/constants/sitemap.ts` (sitemap pipeline marketing pages register into)
...
- (Future) `_prototypes/marketing-pages-templates.html` — high-fidelity visual reference for the three template kinds. **Not yet created.** Recommended: build before or alongside Story 26.1 so the section-component library has a visual source-of-truth. Open question.
...
- **Current relevant functionality:** Marketing site lives under `app/(marketing)/page.tsx` (the single homepage) plus a small set of public legal/about pages (`app/(public)/villkor`, `/integritetspolicy`, `/cookiepolicy`, `/personuppgiftsbitradesavtal`, `/underbitraden`, `/om-oss`, `/kontakt`). Shared chrome via `components/shared/navigation/{navbar,footer}.tsx`. Brand fonts (Safiro headers, Google Sans Flex body) declared in `app/globals.css`. Warm off-white palette with sage secondary. Public catalog routes already live: `/lagar/[id]`, `/eu/[type]/[id]`, `/alla-lagar/[lawSlug]`, plus browse pages under `/browse/*` and `/rattskallor`. Sitemap pipeline (Next 16 `generateSitemaps()` + `/sitemap-index.xml` route) handles ~10k catalog URLs via 10k-per-chunk segments.
...
- **Integration points:**
  - `app/sitemap.ts`: extended to walk the MDX content folder and emit one entry per marketing page (no per-page manual registration). Sitemap chunking already exists; marketing pages will sit alongside catalog URLs in the same chunk pool.
...
- **What's being added/changed:**
  ...
  2. **Section component library** — eight reusable sections in `components/marketing/sections/`: `<MarketingHero>`, `<FeatureGrid>`, `<ProofBlock>`, `<CtaBlock>`, `<CatalogLawList>`, `<ChangeFeedEmbed>`, `<FaqAccordion>`, `<RelatedPagesGrid>`. Each section has a tight prop signature with TypeScript strict typing.
  ...
  6. **Metadata + JSON-LD bundle** — `generateMarketingMetadata()` returns a full `Metadata` object including canonical URL (uses `NEXT_PUBLIC_BASE_URL` = `https://www.laglig.se`), OG image URL (points at the generated `/og-image/[kind]/[slug]` route), `BreadcrumbList` + `Article` (or `FAQPage`) JSON-LD scripts.
  7. **OG image generator** — `app/(marketing)/og-image/[kind]/[slug]/route.tsx` returns a 1200×630 PNG via `ImageResponse`, rendering page title + a kind-specific accent (sage for feature, warm-orange for industry, neutral grey for topic), Safiro font (loaded via inline `@font-face`-equivalent in the `ImageResponse` font option), Laglig logo bottom-left.
  8. **UTM-tagged CTA system** — `<CtaBlock>` accepts a `placement` prop (e.g. `"hero"`, `"mid-page"`, `"footer-strip"`) and emits CTA URLs with `utm_source=marketing&utm_medium=organic&utm_campaign={kind}-{slug}&utm_content={placement}`. GA4 tracks CTA clicks via the existing `gtag` channel (consent-gated).
  9. **Sitemap registration** — `app/sitemap.ts` walks `content/marketing/` at build time and emits one entry per MDX file. Lastmod from file mtime. Priority 0.8 (between catalog at 0.7 and homepage at 1.0).
  10. **Section content matching brand** — every page renders inside the existing `<Navbar>` + `<Footer>` chrome. No new fonts, no new colors. The visual language follows the existing landing page hero treatment.
- **How it integrates:**
...
- AC 3: Eight section components built with TypeScript-strict props (`<MarketingHero>`, `<FeatureGrid>`, `<ProofBlock>`, `<CtaBlock>`, `<CatalogLawList>` — stub for 26.3, `<ChangeFeedEmbed>`, `<FaqAccordion>`, `<RelatedPagesGrid>`)
...
- AC 8: UTM-tagged CTA system in `<CtaBlock>` with `placement` prop
- AC 9: `app/sitemap.ts` extended to walk `content/marketing/` and auto-register
...
- AC 2: `<CatalogLawList>` server component takes resolved entries, renders styled `<ul>` with links into `/lagar/[slug]` (or `/eu/{forordningar|direktiv}/[slug]`)
...
- AC 5: Each has BreadcrumbList JSON-LD + an Article (or industry-appropriate) JSON-LD
...
`/branscher/{fastighet, vard-omsorg, industri, transport}`. Once 26.4 proves the template, these parallelize. Each ~1000 words, 10–15 catalog links, full metadata. **Acceptance: same as 26.4 ACs 1–8, scoped to these 4 pages.**
...
`/funktioner/{laglista, kravpunkter, lagandringar, uppgifter, styrdokument}`. ~1000 words each. **Acceptance: same as 26.4 ACs 1–8, scoped to these 5 pages.**
...
**ACs:** same as 26.4 ACs 1–8 scoped + AC 9: each topic page embeds the live `<ChangeFeedEmbed>` for its topic area.
...
- AC 5: Standard metadata + JSON-LD + UTM CTAs
...
3. **Marketing prototype HTML** in `_prototypes/marketing-pages-templates.html`: build-it-now (recommended) or skip? **PO recommendation: build before 26.1 starts.** ~1 day of design exploration that prevents 4–8 days of template rework later. Treat as a Story 0 / Story 26.0 pre-work outside the formal story count.
