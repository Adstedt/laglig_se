...
- `_prototypes/onboarding-tutorial-modal.html` — high-fidelity 6-frame visual reference: path-choice modal, generate-kickoff, import-upload, tutorial-while-working with 6 tab content panels, generate done state, import done state. Real Safiro fonts, real warm off-white palette, realistic mini-mockups of every product surface. **Known gaps (intentional — PO decision 2026-05-13: prototype will NOT be refreshed):** (1) frame ① shows two cards (Generera + Importera) and predates the three-card decision — B.1 implementation must source the three-card pattern from `<CreateListChooser>` (`components/features/document-list/create-list-chooser.tsx`) instead of the prototype frame ①; (2) the prototype has no frame for the Feedback tab (Story B.5) — that step's visual design is driven by the architecture spec + Tailwind primitives, no prototype dependency; (3) the prototype has no frame for the corner FAB / Hjälp sidebar (Story B.6) — same: architecture spec drives. Devs working B.1/B.5/B.6 should treat the architecture doc as visual source of truth for these areas and the prototype as reference for the unchanged frames (②, ③, ④a, ④b).
