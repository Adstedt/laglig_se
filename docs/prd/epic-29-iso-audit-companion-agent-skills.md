...
- **Epic 23** (avvikelser first-class + ISO cycle upgrades — **re-scoped 2026-07-23**) — **the foundation layer this epic sits on.** Per `docs/sprint-change-proposal-avvikelser-capa-2026-07-23.md`, the two epics form one domain in two layers: **Epic 23 = foundation** (finding-as-hub model — `cycle_id` nullable + `SetNull`, `workspace_id`, `source`, no anchor CHECK; the `/avvikelser` register; ISO cycle upgrades; and the *mechanical* agent read tools `list_findings` + `get_finding` edge readers), **Epic 29 = skills layer** (the conversational workflows that orchestrate those tools). The relationship is now a **hard dependency for the skill stories** (29.2/29.4/29.6): they are materially stronger on the richer model (sourced, cross-cycle, recurrence-aware findings) and should be drafted *after* Epic 23 Phase 1 lands — building them on today's cycle-only findings means building them twice. **Unaffected** by the re-scope and buildable now: 29.1 (done), 29.2a (`ask_user`), 29.3 (`create_cycle`), 29.5 (`CYCLE` context). Concrete seams: 29.1's `get_finding` already returns the nullable-cycle-handle shape 23 needs (built forward-compatibly); 29.6's `create_finding` dispatch targets the loosened standalone `createFinding`; and Epic 23's kickoff carry-forward ("kända vid start" / "öppna från tidigare kontroller") and 29's A3 anchor **share one query definition** of "open finding relevant to scope X" — defined once in Epic 23, consumed by both. A10's EXTERN-cycle container for external audit results is unchanged (findings can be cycle-less but the auditor's system-level avvikelser still hang on the EXTERN cycle).
...
### Regime generalization — ISO is the vocabulary, not the substance

The three moments are **not ISO-exclusive**: their non-certified equivalents are legally mandatory in Sweden. Every employer must run SAM (AFS 2001:1) including its **årliga uppföljning** (≈ kontroll + genomgång); verksamhetsutövare owe **egenkontroll** under miljöbalken (26 kap. 19 § + förordning 1998:901); and **tillsynsbesök** (Arbetsmiljöverket, kommunens miljökontor, IMY) are the uncertified company's extern revision — same "visa er dokumentation" dynamic, higher stakes (förelägganden/viten, not closable avvikelser). Market-wise: ISO certification covers low tens of thousands of Swedish companies; SAM covers every employer. The ISO mode wins the auditor channel; the tillsyn/SAM mode serves the long tail.

**Mechanism (cheap by construction — the 19.8 type-module pattern applied to regimes):** each skill keeps ONE procedure core; regime framing modules (`types/iso.md`, `types/tillsyn.md`, `types/sam.md`) select vocabulary (revisionsspråk vs tillsynsspråk), activation triggers (*"vi har revision"* and *"Arbetsmiljöverket kommer på inspektion"* activate the same skill), the anchor evidence (sealed kontroll report vs SAM-dokumentation vs egenkontrollprogram), and the protokoll framing in `ledningens_genomgang` (9.3-structured vs årlig-uppföljning-structured). Regime resolved from company context (certifications in the company profile; Epic 15/27 data) with `ask_user` chips as fallback. A tillsynsbesök records as an `EXTERN` cycle like any certification audit — **zero schema impact**. Stories 29.2 and 29.4 ship the ISO module as primary + the tillsyn/SAM modules as thin variants; the clause table below maps the ISO mode.
...
4. **Story 29.4 — `ledningens_genomgang` skill + period reads.** New/extended read for "changes in period incl. assessments" (extend the unassessed-changes loader or add `list_changes_in_period` — implementer's call, same conventions); skill directory encoding Journey B (period anchor via decision log, 9.3.2 assembly per the mapping table, placeholder honesty, B6 loop-close); protokoll via `draft_styrdokument` as `REPORT` type with a genomgång type module (19.8 pattern). Regime modules per the Regime generalization section: `types/iso.md` (9.3-structured protokoll) + `types/sam.md` (årlig-uppföljning framing for uncertified employers — same assembly, different protokoll skeleton). **No schema changes.**
