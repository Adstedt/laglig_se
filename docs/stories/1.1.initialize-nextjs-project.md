...
- [ ] **Task 1: Initialize Next.js 16 Project** (AC: 1)
  - [ ] Install pnpm globally if not present: `npm install -g pnpm@9`
  - [ ] Verify pnpm version: `pnpm --version` (should be 9.0+)
  - [ ] Run `pnpm create next-app@latest laglig-se --typescript --tailwind --app --eslint`
  - [ ] Verify Next.js 16+ is installed (check `package.json`)
  - [ ] Verify App Router is selected (app/ directory exists, NOT pages/)
  - [ ] Verify Turbopack is default bundler (Next.js 16 includes it automatically)
  - [ ] Test that dev server starts: `pnpm dev`

- [ ] **Task 2: Configure TypeScript Strict Mode** (AC: 2, 7)
  - [ ] Update `tsconfig.json` with complete strict configuration:
    - `"strict": true`
    - `"noImplicitAny": true`
  ...
  - [ ] Configure TypeScript path aliases in `tsconfig.json`:
    ...
    ```
  - [ ] Run `pnpm typecheck` (or `tsc --noEmit`) to verify no type errors

- [ ] **Task 3: Configure Tailwind CSS with Custom Tokens** (AC: 3)
  - [ ] Update `tailwind.config.ts` to add custom color tokens with full scale:
    ```typescript
    theme: {
      extend: {
        colors: {
    ...
    ```
  - [ ] Verify Tailwind CSS directives are present in `styles/globals.css`:
    ```css
    @tailwind base;
    @tailwind components;
    @tailwind utilities;
    ```
  - [ ] Test custom colors by adding a sample component using `bg-primary-500`, `text-success-500`, etc.

- [ ] **Task 4: Configure ESLint and Prettier** (AC: 4, 7)
  ...
  - [ ] Create `.prettierrc` file:
    ...
    ```
  - [ ] Update `.eslintrc.json` to include accessibility rules:
    ```json
    {
    ...
    ```
  - [ ] Install ESLint accessibility plugin: `pnpm add -D eslint-plugin-jsx-a11y`
  - [ ] Install eslint-config-prettier: `pnpm add -D eslint-config-prettier`
  - [ ] Add lint and format scripts to `package.json`:
    ...
    ```
  - [ ] Run `pnpm lint` and `pnpm format` to verify no errors

- [ ] **Task 5: Configure Git Hooks with Husky and lint-staged** (AC: 4, 5)
  - [ ] Install Husky and lint-staged: `pnpm add -D husky@9 lint-staged@15`
  ...
  - [ ] Add lint-staged configuration to `package.json`:
    ...
    ```
  - [ ] Test git hooks by making a dummy commit

- [ ] **Task 6: Initialize Git Repository** (AC: 5)
  - [ ] Verify Git repository is initialized (should be done by `create-next-app`)
  ...
  - [ ] Create initial commit: `git add . && git commit -m "feat(init): initialize Next.js 16 project with TypeScript and Tailwind"`

- [ ] **Task 7: Verify Development Server** (AC: 6, 7)
  - [ ] Start development server: `pnpm dev`
  - [ ] Verify server starts on `http://localhost:3000`
  - [ ] Verify default Next.js homepage renders without errors
  - [ ] Check browser console for any errors or warnings
  - [ ] Verify Turbopack is being used (should see "Turbopack" in terminal output)

- [ ] **Task 8: Run Build and Verify No Errors** (AC: 7)
  - [ ] Run production build: `pnpm build`
  - [ ] Verify build completes successfully with no TypeScript errors
  - [ ] Verify build completes successfully with no ESLint errors
