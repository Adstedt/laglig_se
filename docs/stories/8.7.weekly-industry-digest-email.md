...
2. **Email Structure:**
   - Section 1: "Ändringar i dina lagar" (Changes to workspace laws)
   - Section 2: "Upptäck relevanta lagar" (Industry starter pack laws NOT in workspace)
...
5. **Section 3 Content:**
   ...
   - CTA: "Läs mer om denna lag"

6. Email sent only if ≥1 change occurred that week (Sections 1 or 2)

7. Subject: "Veckoöversikt: [N] lagändringar + nya rekommendationer"
        ...
        <Container style={{ maxWidth: '600px', margin: '0 auto', backgroundColor: '#ffffff' }}>
          ...
          <Section style={{ padding: '24px' }}>
            ...
            <Text style={{ fontSize: '14px', color: '#6b7280', margin: '0' }}>
              Här är vad som hänt den senaste veckan inom <strong>{workspaceName}</strong> och din bransch.
            </Text>
          </Section>
            ...
            <>
              ...
              {workspaceChanges.map((change) => (
                ...
                >
                  ...
                  >
                    <strong>Påverkan:</strong> {change.businessImpact}
                  </Text>
                  <Link
                    href={change.diffUrl}
                    style={{
            ...
            <>
              ...
              <Section style={{ padding: '0 24px' }}>
                <Text style={{ fontSize: '18px', fontWeight: 'bold', color: '#111827', margin: '0 0 8px 0' }}>
                  🔍 Upptäck relevanta lagar
                </Text>
                <Text style={{ fontSize: '14px', color: '#6b7280', margin: '0 0 16px 0' }}>
                  Dessa lagar är relevanta för din bransch men finns inte i din lista än.
                </Text>
              </Section>
              ...
              {discoveryLaws.map((law) => (
                ...
                >
                  ...
                  <Text style={{ fontSize: '13px', color: '#854d0e', margin: '0 0 12px 0' }}>
                    {law.changeCount} {law.changeCount === 1 ? 'ändring' : 'ändringar'} den senaste veckan
                  </Text>
                  <Button
          ...
          {aiRecommendations.length > 0 && (
            <>
              ...
              <Section style={{ padding: '0 24px' }}>
                ...
                <Text style={{ fontSize: '14px', color: '#6b7280', margin: '0 0 16px 0' }}>
                  Baserat på din verksamhet rekommenderar vi dessa lagar:
                </Text>
              </Section>
              ...
              {aiRecommendations.map((rec) => (
                ...
                >
                  ...
                  <Text style={{ fontSize: '13px', color: '#166534', margin: '0 0 12px 0' }}>
                    <strong>Varför:</strong> {rec.reason}
                  </Text>
                  <Link
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const workspace of workspaces) {
      ...
      // Section 1: Workspace changes
      const workspaceChanges = workspace.workspaceLaws
        .flatMap((wl) =>
          wl.law.lawChanges.map((change) => ({
            ...
            aiSummary: change.aiSummary || 'Ändring upptäckt.',
            businessImpact: change.businessImpact || 'Kräver granskning',
      ...
      // Section 3: AI recommendations
      const aiRecommendations = await generateWeeklyRecommendations(workspace.id)

      // Send email
      try {
        await sendEmail({
          to: workspace.owner.email,
          subject: `Veckoöversikt: ${workspaceChanges.length + discoveryLaws.reduce((sum, law) => sum + law.changeCount, 0)} lagändringar + nya rekommendationer`,
        ...
        console.error(`Failed to send weekly digest to ${workspace.owner.email}:`, error)
      }
    }
...
- Email sent only if ≥1 change in Sections 1 or 2
