...
- [ ] **Task 1: Prisma schema — ContentChunk model** (AC: 1-7, 9)
  - [ ] Add `SourceType` enum: `LEGAL_DOCUMENT`, `USER_FILE`, `CONVERSATION`, `ASSESSMENT`
  - [ ] Add `ContentRole` enum: `STYCKE`, `ALLMANT_RAD`, `TABLE`, `HEADING`, `TRANSITION_PROVISION`, `FOOTNOTE`, `MARKDOWN_CHUNK`
  - [ ] Add `ContentChunk` model with all fields per AC 1
  - [ ] Add `@@index([source_type, source_id])` and `@@index([workspace_id])`
  - [ ] Add `@@map("content_chunks")`
  - [ ] Run `pnpm prisma migrate dev --name add-content-chunk-model`
  - [ ] Run `pnpm prisma generate`

- [ ] **Task 2: pgvector HNSW index** (AC: 8)
  - [ ] Add raw SQL to the migration file for HNSW index:
    ```sql
    CREATE INDEX content_chunks_embedding_idx ON content_chunks
      USING hnsw (embedding vector_cosine_ops)
      WITH (m = 16, ef_construction = 64);
    ```
  - [ ] Verify index created by checking migration output

- [ ] **Task 3: Token counting utility**
  - [ ] Create `lib/chunks/token-count.ts`
  - [ ] Implement `estimateTokenCount(text: string): number` — use `Math.ceil(text.length / 4)` heuristic
  - [ ] Export for use in chunking function and embedding cost estimation (Story 14.3)

- [ ] **Task 4: Chunking function — Paragraf-level** (AC: 12-13)
  - [ ] Create `lib/chunks/chunk-document.ts` and `lib/chunks/index.ts` (barrel re-exports)
  - [ ] Implement `chunkDocument()` that iterates canonical JSON: `chapters → paragrafer`
  - [ ] Each **paragraf (§)** becomes one chunk — concatenate all stycken text into a single `content` string (the § is the semantic unit)
  - [ ] For each paragraf, generate: sourceType, sourceId, workspaceId (null), path, contextualHeader, content, contentRole, tokenCount
  - [ ] `contentRole`: use the dominant role from the paragraf's stycken (typically `STYCKE`; if all stycken are `ALLMANT_RAD` or `TABLE`, use that role)
  - [ ] Path encoding: `kap{chapter.number}.§{paragraf.number}` — use `kap0` for implicit chapters
  - [ ] Contextual header: `"{title} ({documentNumber}) > Kap {N}: {chapterTitle} > {paragraf.number} §"` — omit chapter for flat docs
  - [ ] Handle edge cases: implicit chapter (null number), paragrafer without stycken (skip, log warning)

- [ ] **Task 5: Chunking function — Non-§ content** (AC: 14)
  - [ ] Chunk `transitionProvisions` (if present): group all stycken into one chunk, path `overgangsbest`, role `TRANSITION_PROVISION`, header = `"{title} ({documentNumber}) > Övergångsbestämmelser"`
  - [ ] Chunk `preamble` (if present): one chunk, path `preamble`, role `STYCKE`, header = `"{title} ({documentNumber}) > Inledning"`
  - [ ] Chunk `appendices` (if present): one chunk per appendix, path `bilaga.{N}`, role `STYCKE`, header = `"{title} ({documentNumber}) > Bilaga {N}"`

- [ ] **Task 6: Chunking function — Markdown fallback** (AC: 15)
  - [ ] In `chunkDocument()`, detect empty JSON (all chapters have 0 paragrafer AND no transition provisions/preamble/appendices)
  - [ ] When empty: derive chunks from `markdown_content` (or `html_content` via `htmlToPlainText()`)
  ...
  - [ ] Assign `contentRole: MARKDOWN_CHUNK`, `path: md.chunk{N}`, contextual header = document title + number

- [ ] **Task 7: Chunk lifecycle sync** (AC: 16)
  - [ ] Create `lib/chunks/sync-document-chunks.ts`
  - [ ] Implement `syncDocumentChunks(documentId: string): Promise<SyncResult>`
  - [ ] Load document from DB with `json_content`, `title`, `document_number`, `content_type`, `markdown_content`, `html_content`
  - [ ] Guard: only process `SFS_LAW` and `AGENCY_REGULATION` content types — skip amendments and other types with a warning
  - [ ] Delete all existing chunks for this document + create new ones in a Prisma `$transaction`
  - [ ] Return `SyncResult`: `{ documentId, chunksDeleted, chunksCreated, duration }`
  - [ ] Handle: document not found, no json_content AND no markdown → log warning, skip

- [ ] **Task 8: Tests** (AC: 17)
  - [ ] Create `tests/unit/chunks/chunk-document.test.ts` (at least 9 tests)
