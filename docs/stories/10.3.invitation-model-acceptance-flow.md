...
- [ ] **Task 4: Integrate invitation check into onboarding page** (AC: 5, 7, 21)
  - [ ] In `app/onboarding/page.tsx` (Server Component), call `getPendingInvitations(user.email)` to check for pending invitations
  - [ ] Deduplicate: if multiple invitations to the same workspace exist, only pass the most recent one to the UI
  - [ ] Pass invitations to the onboarding wizard component
  - [ ] In `onboarding-wizard.tsx`, if invitations are present, show `PendingInvitations` component first. If user dismisses all (declines all or clicks "Skapa eget workspace"), switch to the wizard steps.
...
- **Server Actions**: `app/actions/` pattern — async functions with `'use server'`, return `{ success, error }` [Source: architecture/17-coding-standards.md#17.3]
...
- **Caching**: `invalidateUserCache()` from workspace context after mutations [Source: lib/auth/workspace-context.ts]
...
app/actions/
  ...
  workspace.ts                        # REFERENCE: Pattern for server actions, cache invalidation
    - Line 191-237: addWorkspaceMember() — REFERENCE for creating WorkspaceMember records
...
lib/auth/
  workspace-context.ts                # READ ONLY: invalidateUserCache(), setActiveWorkspace()
    - Line 261-269: setActiveWorkspace() — sets httpOnly cookie
    - invalidateUserCache pattern for cache clearing
...
- Server actions in `app/actions/` with `'use server'` directive [Source: architecture/17-coding-standards.md#17.3]
