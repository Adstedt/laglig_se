...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < laws.length; i += BATCH_SIZE) {
      ...
      console.log(
        `Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(laws.length / BATCH_SIZE)}`
      )

      // Process batch in parallel with concurrency limit
      const limit = pLimit(CONCURRENCY_LIMIT)
      const promises = batch.map((law) => limit(() => processLaw(law, stats)))
    ...
    console.log(
      `Change detection completed in ${durationMinutes.toFixed(1)} minutes`
    )
...
async function processLaw(law: any, stats: JobStats): Promise<void> {
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      retries++
      console.error(
        `Failed to process law ${law.sfsNummer} (attempt ${retries}):`,
        error
      )
