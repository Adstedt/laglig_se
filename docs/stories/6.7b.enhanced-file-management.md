...
- [ ] **Task 1: Add folder support to database schema** (AC: 1, 2, 3)
  - [ ] Create migration adding `parent_folder_id` column to `workspace_files` table
  - [ ] Add `is_folder` boolean column to distinguish folders from files
  - [ ] Add unique constraint: `(workspace_id, parent_folder_id, filename)` for same-folder uniqueness
  - [ ] Add recursive CTE query helper for folder tree operations
  - [ ] Add `path` computed/stored field for efficient breadcrumb queries
  - [ ] Update Prisma schema and generate client

- [ ] **Task 2: Create folder server actions** (AC: 1, 6, 7, 8, 27, 28, 30)
  - [ ] Create `createFolder(name, parentFolderId?)` - create folder at path
  - [ ] Create `renameFolder(folderId, newName)` - rename folder
  - [ ] Create `moveItem(itemId, targetFolderId)` - move file or folder
  - [ ] Create `deleteFolder(folderId)` - delete folder (with contents check)
  - [ ] Create `getFolderTree(workspaceId)` - get full folder tree for sidebar
  - [ ] Create `getFolderContents(folderId, filters, pagination)` - list folder contents
  - [ ] Create `getFolderPath(folderId)` - get breadcrumb path array
  - [ ] Create `getFolderSize(folderId)` - aggregate size of folder contents
  - [ ] All actions use `withWorkspace()` pattern
  - [ ] **Security:** Add path traversal validation - reject paths containing `..` or `.`
  - [ ] **Security:** Validate folder names don't contain path separators (`/`, `\`)

- [ ] **Task 3: Update file upload to support folder context** (AC: 31)
  - [ ] Modify `uploadFile(formData)` to accept `parentFolderId` parameter
  - [ ] Update file path storage to include folder structure
  - [ ] Validate folder exists and belongs to workspace before upload

### Frontend Components

- [ ] **Task 4: Create folder tree sidebar component** (AC: 4, 29, 35)
  - [ ] Create `components/features/files/folder-tree.tsx`
  - [ ] Use Radix UI `Collapsible` for expandable tree nodes
  - [ ] Lazy load folder contents on expand (don't load full tree upfront)
  - [ ] Highlight current folder in tree
  - [ ] Drag-and-drop target highlighting for move operations
  - [ ] Folder icon with thumbnail grid preview for folders with images
  - [ ] Recent folders section at top of tree
  - [ ] **Accessibility:** Add `aria-expanded`, `aria-label` for folder nodes
  - [ ] **Accessibility:** Keyboard navigation (Tab between nodes, Arrow keys to expand/collapse, Enter/Space to select)
  - [ ] **Accessibility:** Focus management when expanding/collapsing folders

- [ ] **Task 5: Implement breadcrumb navigation** (AC: 5, 34)
  - [ ] Create `components/features/files/folder-breadcrumb.tsx`
  - [ ] Clickable segments for each path level
  - [ ] "..." truncation for deep paths (show first and last 2)
  - [ ] Home icon for root ("Mina dokument")
  - [ ] Keyboard shortcut Alt+Up for parent folder

- [ ] **Task 6: Update documents page for folder routing** (AC: 32, 33)
  - [ ] Convert `app/(workspace)/documents/page.tsx` to dynamic route
  - [ ] Create `app/(workspace)/documents/[...path]/page.tsx`
  - [ ] Parse path segments to resolve folder
  - [ ] Update `loadFiles` to fetch folder contents instead of all files
  - [ ] Add folder/file rendering in grid and list views
  - [ ] Implement double-click navigation into folders

- [ ] **Task 7: Enhance list view with additional columns** (AC: 9, 10, 11, 12, 13)
  - [ ] Add columns: "Senast ändrad", "Typ", "Plats"
  - [ ] Create `getFileTypeLabel(mimeType)` helper for Swedish labels
  - [ ] Implement column resize with drag handles
  - [ ] Persist column widths in localStorage
  - [ ] Add row hover thumbnail preview for images
  - [ ] Update sorting to support new columns

- [ ] **Task 8: Create folder context menu** (AC: 28)
  - [ ] Create `components/features/files/folder-context-menu.tsx`
  - [ ] Use shadcn/ui `ContextMenu` component
  - [ ] Actions: Öppna, Byt namn, Flytta till, Radera
  - [ ] Inline rename mode with Enter to confirm, Escape to cancel

### Rich Preview Components

- [ ] **Task 9: Implement image preview with zoom** (AC: 17, 22, 23)
  - [ ] Install `react-zoom-pan-pinch` library
  - [ ] Create `components/features/files/preview/image-preview.tsx`
  - [ ] `TransformWrapper` with zoom/pan controls
  - [ ] Custom toolbar: zoom in, zoom out, reset, fullscreen
  - [ ] Touch gesture support (pinch-to-zoom)
  - [ ] Download signed URL from Supabase for image src
  - [ ] Error state: "Kunde inte läsa in bilden" with retry button
  - [ ] Loading state: Skeleton placeholder during image load

- [ ] **Task 10: Implement PDF preview** (AC: 18)
  - [ ] Install `react-pdf` library
  - [ ] Configure PDF.js worker for Next.js (dynamic import with ssr: false)
  - [ ] Create `components/features/files/preview/pdf-preview.tsx`
  - [ ] Render first page by default with page navigation
  - [ ] Page number input for jumping to specific page
  - [ ] Zoom controls similar to image preview
  - [ ] "Öppna i ny flik" button using signed URL
  - [ ] Error state: Handle load failures, password-protected PDFs
  - [ ] Loading state: Skeleton with page dimensions hint

- [ ] **Task 11: Implement Office document preview** (AC: 19)
  - [ ] Install `react-office-viewer` library
  - [ ] Create `components/features/files/preview/office-preview.tsx`
  - [ ] Detect file type and render appropriate viewer
  - [ ] Configure locale to "en" (Swedish not supported by library - see Dev Notes)
  - [ ] Fallback UI for unsupported Office formats: "Ladda ner för att visa"
  - [ ] Loading state while document renders
  - [ ] Error state: Handle corrupted files, unsupported versions

- [ ] **Task 12: Create full-screen lightbox** (AC: 17 - helskärm)
  - [ ] Install `yet-another-react-lightbox` with zoom plugin
  - [ ] Create `components/features/files/preview/lightbox.tsx`
  - [ ] Keyboard navigation (arrows, escape)
  - [ ] Zoom plugin enabled for detailed viewing
  - [ ] Caption showing filename and metadata

- [ ] **Task 13: Expand file preview panel** (AC: 15, 16, 20)
  - [ ] Update `file-preview-panel.tsx` width to 640px default
  - [ ] Add expand button to toggle 50% viewport width mode
  - [ ] Persist expanded state in localStorage
  - [ ] Replace icon-based preview with rich preview components
  - [ ] Conditional rendering based on file type
  - [ ] Remember last viewed file ID when changing folders

- [ ] **Task 14: Implement quick preview (spacebar)** (AC: 21, 22, 23)
  - [ ] Create `components/features/files/quick-preview.tsx`
  - [ ] Modal overlay with file preview content
  - [ ] Global keyboard listener for spacebar when file selected
  - [ ] Arrow key navigation between files
  - [ ] Click outside or ESC to dismiss
  - [ ] Minimal UI - just preview content and close button

- [ ] **Task 15: Add extended metadata display** (AC: 25, 26)
  - [ ] Create `components/features/files/preview/metadata-panel.tsx`
  - [ ] Extract image dimensions server-side or client-side
  - [ ] PDF page count via react-pdf `onLoadSuccess`
  - [ ] EXIF extraction using `exif-js` library for photos
  - [ ] Extract: camera model, date taken, GPS coordinates (if available), orientation
  - [ ] Collapsible "Egenskaper" section with all metadata
  - [ ] Swedish labels for all metadata fields

### Integration

- [ ] **Task 16: Update file upload flow for folder context** (AC: 31)
  - [ ] Modify `FileDropzone` to accept current folder ID prop
  - [ ] Update upload to pass folder context to server action
  - [ ] Show current folder name in upload zone: "Ladda upp till [Folder]"

- [ ] **Task 17: Implement drag-and-drop move** (AC: 6)
  - [ ] Add `draggable` attribute to file/folder cards
  - [ ] Folder tree nodes as drop targets
  - [ ] Visual feedback during drag (ghost element, valid drop highlight)
  - [ ] Move confirmation toast with undo option
  - [ ] Prevent moving folder into itself (cycle detection)

- [ ] **Task 18: Create "Flytta till" modal** (AC: 6, 28)
  - [ ] Create `components/features/files/move-to-modal.tsx`
  - [ ] Folder tree picker for destination
  - [ ] Create new folder inline option
  - [ ] "Flytta" button to confirm
  - [ ] Works for single item or bulk selection

### Testing

- [ ] **Task 19: Unit tests** (All ACs)
  - [ ] Test folder CRUD operations
  - [ ] Test path computation and breadcrumb generation
  - [ ] Test move operation cycle detection
  - [ ] Test preview component rendering for each file type
  - [ ] Test keyboard shortcuts and quick preview
