...
- [ ] **Task 6: Implement chat history persistence** (AC: 9)
  - [ ] Store messages in `chat_messages` table via Server Action
  - [ ] Load chat history on sidebar open
  - [ ] Scope history to workspace_id
...
5. **Task 6 Skipped:** Chat history persistence requires database schema changes (`chat_messages` table). This task should be picked up in a follow-up story or when database migrations are addressed.
