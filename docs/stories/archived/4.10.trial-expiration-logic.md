...
> **Superseded by Story 5.13** (Trial Expiration Hard Gate + Stripe Checkout Conversion) on 2026-05-06. This story was drafted Nov 2025 against a hypothetical schema before Stripe shipped — its camelCase field names (`trialEndsAt`, `subscriptionTier='trial'`), Next.js middleware approach, `node-cron` scheduler, custom marketing-style trial-expired page, and EUR pricing all conflict with what 5.4 / 5.5\* / 5.12 actually built. Retained as historical context (the lifecycle mental model — cron + email + access gate + data preservation — carried forward into 5.13). See `docs/stories/5.13.trial-expiration-hard-gate.md` for the active spec.
...
export function scheduleTrialExpirationJob() {
  cron.schedule('0 0 * * *', async () => {
    ...
    try {
      ...
      console.log(
        `Found ${workspacesToDelete.length} workspaces to soft-delete`
      )
