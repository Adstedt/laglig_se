...
Archived — Superseded by Epic 14. The agent tool + action-card system delivers all of 3.8's scope and more: `search_laws` + `add_laws_to_list` (law cards / "Add to list" — 14.7a/14.7c), `create_task` ("Create task" — 14.7c/14.22), interactive in-chat cards via the `PendingAgentAction` action-card renderers (14.22/14.23/14.24), and tool-call streaming on the current Vercel AI SDK. 3.8's code sketch (`ai/react` `useChat` + `experimental_onToolCall`) is an obsolete SDK pattern. **No residual.**
...
9. Example: "What HR laws apply to restaurants?" → AI streams 5 law cards
