...
Archived — **Partially** superseded by Epic 14. The grounding / cite-first *mechanism* shipped: `[Källa: …]` citation markers + pill resolution (`lib/ai/citations.ts`, Story 14.16) and the "answer only from provided sources / 'Jag har inte tillräcklig information'" steering (Story 14.9, system-prompt behavioral design — AC 7 here). The *measurement & eval* half — response-grounding logging (query + retrieved chunks + answer), hallucination detection, a grounding-rate dashboard (<5% target), and the edge-case eval set — was **NOT** built and is extracted to new **Story 19.13** (Agent Answer-Grounding Eval). See 19.13.
...
7. System instruction enforced: "If you cannot answer from provided sources, respond 'Jag har inte tillräcklig information...'"
