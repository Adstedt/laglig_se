...
Archived — Superseded by Epic 14 (Stories 14.2 + 14.3 + 14.8). pgvector extension active, HNSW index on `ContentChunk.embedding` (14.2), 228K chunks embedded (14.3), retrieval function `retrieveContext()` defined in Story 14.8. Storage uses unified `ContentChunk` table (not `law_embeddings` proposed here).
