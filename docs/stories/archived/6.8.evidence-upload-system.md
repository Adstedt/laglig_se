...
The dedicated `evidence` table approach in this draft was not implemented as written. Evidence/attachment functionality was delivered through a different architecture:

- **`WorkspaceFile`** — underlying file model backed by Supabase Storage (workspace-scoped RLS)
- **`FileTaskLink`** — files attached to tasks via join table (task attachments)
- **`RequirementEvidenceLink`** (Story 17.18) — evidence attached at the **kravpunkt (requirement)** level rather than directly to tasks, with a `bevis_required` flag driving the gap signal
- **`LinkedArtifactsPanel`** (Story 17.18) — consolidated UI surfacing evidence, task attachments, and document links for a list item

See `docs/stories/completed/17.18.*` for the canonical evidence flow and `prisma/schema.prisma` for the current data model.
