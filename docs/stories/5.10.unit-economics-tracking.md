...
model WorkspaceCost {
  ...
  aiApiCost        Float    @default(0) // €
  vectorQueryCost  Float    @default(0) // €
  storageCost      Float    @default(0) // €
  totalCost        Float    @default(0) // €
  revenue          Float    @default(0) // € (subscription MRR)
  grossMargin      Float?   // %
...
model ApiCostLog {
  ...
  inputTokens     Int
  outputTokens    Int
  cost            Float    // € calculated based on model pricing
...
export const VECTOR_QUERY_COST_PER_QUERY = 0.0001 // €0.0001 per query

// Storage pricing (estimated)
export const STORAGE_COST_PER_GB_MONTH = 0.02 // €0.02 per GB per month
...
export function scheduleMonthlyCostAggregation() {
  ...
  cron.schedule('0 3 1 * *', async () => {
    ...
    for (const workspace of workspaces) {
      ...
      const vectorQueryCost =
        (usage?.aiQueriesThisMonth || 0) * VECTOR_QUERY_COST_PER_QUERY

      // Calculate storage costs
      const storageCost =
        ((usage?.storageUsedMb || 0) / 1024) * STORAGE_COST_PER_GB_MONTH
      ...
      console.log(
        `Workspace ${workspace.id}: Revenue €${revenue.toFixed(2)}, Cost €${totalCost.toFixed(2)}, Margin ${grossMargin.toFixed(1)}%`
      )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          title="Total MRR"
          value={`€${data.totalMRR.toFixed(0)}`}
        />
        <MetricCard
          title="Total Costs"
          value={`€${data.totalCosts.toFixed(0)}`}
        />
        <MetricCard
            ...
            <div key={tier.tier}>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-sm text-gray-600">
                  {tier.margin.toFixed(1)}% • {tier.count} workspaces
                </span>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 capitalize">
                      {workspace.tier} • {workspace.margin.toFixed(1)}% margin
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-medium">
                      €{workspace.revenue.toFixed(0)} MRR
                    </p>
                    <p className="text-sm text-red-600">
                      €{workspace.cost.toFixed(2)} cost
                    </p>
                  </div>
                </div>
  ...
  await sendEmail({
    ...
    html: `
      ...
      <ul>
        ...
        <li>Total MRR: €${data.totalMRR.toFixed(0)}</li>
        <li>Total Costs: €${data.totalCosts.toFixed(0)}</li>
      ...
      ${
        ...
        <ul>
          ${data.lowMarginWorkspaces
            .map(
              (ws: any) => `
            <li>${ws.name}: ${ws.margin.toFixed(1)}% (€${ws.cost.toFixed(2)} cost / €${ws.revenue.toFixed(0)} revenue)</li>
          `
            )
            .join('')}
        </ul>
