...
### Follow-up: Content Inheritance Refactor

**Problem:** Templates currently store their own `compliance_summary` and `expert_commentary` per item, duplicating data that should live on `LegalDocument` (`summary`, `kommentar`). The detail modal shows `LawListItem.ai_commentary` (copied from template) instead of the canonical `LegalDocument.summary`/`kommentar` fields.

**Proposed refactor:**
- **Document-level (shared):** `LegalDocument.summary` + `LegalDocument.kommentar` displayed everywhere — the canonical AI summaries for a law
- **Template-level (domain context):** Replace `compliance_summary`/`expert_commentary` with a `template_context` field — domain-specific relevance ("why this law matters for this compliance area")
- **List item-level (user-editable):** `businessContext`, `compliance_actions`, etc. remain per-workspace user fields

This eliminates redundancy, ensures all views show the same document summaries, and gives templates a focused role: adding domain context rather than duplicating generic summaries.
...
| 2026-02-09 | Follow-up flagged | Content inheritance refactor: templates should use LegalDocument.summary/kommentar instead of duplicating, add template_context for domain-specific relevance |
...
- Story has 9 acceptance criteria (>5 threshold)
- 12 files touched across schema, server action, UI, and tests

### Code Quality Assessment

The implementation is structurally sound. The server action follows established patterns well: Zod validation, workspace resolution via `withWorkspace()`/`requireWorkspaceAccess()`, transactional bulk insert, and `revalidatePath` for cache invalidation. The component architecture correctly respects the server/client boundary — workspace data is fetched server-side and threaded as props through the component tree.

However, **all server action tests are broken** due to a mock mismatch, and the error handling deviates from the established codebase pattern.

### Findings

**Finding 1 (HIGH): Test Mock Mismatch — All Server Action Tests Fail**

The server action at `app/actions/template-adoption.ts:15` imports `getPublishedTemplateBySlugUncached`, but both test files mock only `getPublishedTemplateBySlug`:

- `tests/unit/actions/template-adoption.test.ts:34` — mocks `getPublishedTemplateBySlug`
- `tests/integration/template-adoption.test.ts:28` — mocks `getPublishedTemplateBySlug`

Vitest v4.x throws a clear error: `No "getPublishedTemplateBySlugUncached" export is defined on the "@/lib/db/queries/template-catalog" mock`. Result: **10 of 13 tests fail** (the 3 passing tests are CTA component tests that don't hit this mock).

**Fix:** In both test files, change the mock factory to export `getPublishedTemplateBySlugUncached` instead of `getPublishedTemplateBySlug`, and update all `vi.mocked()` references accordingly.

**Finding 2 (MEDIUM): Error Message Leakage in Catch Block**

At `app/actions/template-adoption.ts:170-171`:
```typescript
const message = error instanceof Error ? error.message : 'Kunde inte adoptera mall'
return { success: false, error: message }
```

This exposes raw `error.message` (which could include internal Prisma errors, auth stack traces, etc.) to the client. The established pattern in `app/actions/document-list.ts` consistently uses fixed Swedish error messages:
```typescript
return { success: false, error: 'Kunde inte adoptera mall' }
```

**Fix:** Remove the `error instanceof Error` branch and use the fixed message directly.

**Finding 3 (LOW): Integration Test is Not a True Integration Test**

`tests/integration/template-adoption.test.ts` mocks all external dependencies (`prisma`, `workspace-context`, `next/cache`, `template-catalog`) identically to the unit tests. It is functionally an elaborate unit test, not an integration test. This is a classification issue — the test provides value but the "integration" label is misleading.

**Recommendation:** Either rename to a unit test or (future) replace with a test that uses a real test database.

### Refactoring Performed

None — test fixes require dev action since they involve correcting mock targets and error handling.

### Compliance Check

- Coding Standards: ✓ — TypeScript strict mode, Zod validation, shadcn/ui components, proper `'use client'`/`'use server'` directives
- Project Structure: ✓ — Files placed in correct directories per `docs/architecture/12-unified-project-structure.md`
- Testing Strategy: ✗ — Test mock mismatch causes 10/13 failures; integration test does not test actual integration
- All ACs Met: ✓ (implementation logic) — All 9 acceptance criteria are addressed in the code. The `TEMPLATE` enum, `metadata` field, CTA button, workspace selector, field mapping, duplicate naming, variant handling, redirect, and toast are all implemented correctly.

### Improvements Checklist

- [ ] Fix test mock: change `getPublishedTemplateBySlug` → `getPublishedTemplateBySlugUncached` in both `tests/unit/actions/template-adoption.test.ts` and `tests/integration/template-adoption.test.ts`
- [ ] Update `vi.mocked()` import references in both test files to use the uncached function
- [ ] Fix error handling: replace `error instanceof Error ? error.message : '...'` with fixed message `'Kunde inte adoptera mall'` in `app/actions/template-adoption.ts:170-171`
- [ ] Verify all 13 tests pass after fixes

### Security Review

- **Auth**: Workspace access properly gated via `withWorkspace()` and `requireWorkspaceAccess()`. No direct workspace ID trust without verification.
- **Input validation**: Zod schema validates `templateSlug` (non-empty string) and `workspaceId` (UUID format, optional).
- **Error exposure** (Finding 2): Raw error messages leaked to client — medium concern. Fix described above.
- No SQL injection risk (Prisma ORM).
- No XSS risk (React auto-escaping, no `dangerouslySetInnerHTML`).

### Performance Considerations

- Bulk insert via `createMany` for items — correct approach for 112+ items.
- `Promise.all` for group creation — good parallel execution.
- Server page uses `Promise.all` for parallel data fetching (template, workspaces, context) — well-optimized.
- `getPublishedTemplateBySlugUncached` correctly avoids React `cache()` for a mutation context.

### Files Modified During Review

None — no files were modified. Fixes are recommended for dev.

### Gate Status

Gate: FAIL → docs/qa/gates/12.10-adopt-template-workspace.yml
Reason: 10 of 13 tests fail due to mock mismatch — must be fixed before this story can pass.

### Recommended Status

✗ Changes Required — See unchecked items above (3 must-fix items).
(Story owner decides final status)

---

### Review Date: 2026-02-10 (Re-review)
...
- Story has 9 acceptance criteria (>5 threshold)
- 12 files touched across schema, server action, UI, and tests
- Previous gate was FAIL

### Code Quality Assessment

All three findings from the initial review have been addressed:

1. **Mock mismatch (FIXED):** Both `template-adoption.test.ts:34` and `template-adoption.test.ts` integration test now correctly mock `getPublishedTemplateBySlugUncached`. All 36 tests pass.
2. **Error leakage (FIXED):** `app/actions/template-adoption.ts:170` now returns fixed message `'Kunde inte adoptera mall'` — no raw `error.message` exposure.
3. **Integration test classification (ADVISORY):** Still a mock-based test, not a true integration test. Low priority, non-blocking.

Implementation quality is solid. The server action follows established patterns (`withWorkspace`, `requireWorkspaceAccess`, `ActionResult<T>`, Zod validation, `$transaction`, `revalidatePath`). The UI respects the server/client boundary — workspace data flows as props from server component through the component tree.

### Requirements Traceability

| AC | Description | Test Coverage |
|----|-------------|---------------|
| 1 | "Använd denna mall" button | `template-adopt-cta.test.tsx`: "renders button", "button is enabled" |
| 2 | Workspace selector for multi-workspace | `template-adopt-cta.test.tsx`: "opens workspace dialog"; `workspace-selector-dialog.test.tsx`: 4 tests |
| 3 | Creates LawList + groups + items | `template-adoption.test.ts`: "creates 1 LawList + N groups + M items", "maps fields correctly", "stores provenance" |
| 4 | Variant handling | `template-adoption.test.ts`: "returns error for 0 items (empty variant)"; variant resolution in `getPublishedTemplateBySlugUncached` |
| 5 | Post-adoption redirect | `template-adopt-cta.test.tsx`: "shows success toast and redirects" → verifies `router.push('/laglistor')` |
| 6 | Toast notification | `template-adopt-cta.test.tsx`: "shows success toast" → verifies toast message format |
| 7 | Independent list | Architectural: data is copied (no FK to template items), verified by field mapping test |
| 8 | Duplicate name suffix | `template-adoption.test.ts`: "(2)" and "(3)" tests; `generate-unique-name.test.ts`: 5 tests |
| 9 | TEMPLATE enum | Schema verified; `template-adoption.test.ts`: "maps fields correctly" → `source: 'TEMPLATE'` |
...
- Testing Strategy: ✓ — 36/36 tests pass; mock mismatch fixed; comprehensive coverage of all ACs
- All ACs Met: ✓ — All 9 acceptance criteria implemented and verified

### Improvements Checklist

- [x] Fix test mock: `getPublishedTemplateBySlug` → `getPublishedTemplateBySlugUncached` (done in prior fix commit)
- [x] Fix error handling: fixed message instead of `error.message` (done in prior fix commit)
- [x] All 36 tests passing (verified)
- [ ] (Future) Reclassify integration test or replace with real DB test
- [ ] (Future) Extend `requireWorkspaceAccess` for true cross-workspace adoption

### Security Review

- **Auth**: Workspace access properly gated via `withWorkspace()` and `requireWorkspaceAccess()`. No direct workspace ID trust without verification.
- **Input validation**: Zod schema validates `templateSlug` (non-empty string) and `workspaceId` (UUID format, optional).
- **Error exposure**: Fixed — catch block now returns fixed Swedish message only. No raw error leakage.
- No SQL injection risk (Prisma ORM).
- No XSS risk (React auto-escaping, no `dangerouslySetInnerHTML`).
...
- Bulk insert via `createMany` for items — correct approach for 112+ items.
- `Promise.all` for group creation — good parallel execution.
- Server page uses `Promise.all` for parallel data fetching (template, workspaces, context).
- `getPublishedTemplateBySlugUncached` correctly avoids React `cache()` for mutation context.
...
Gate: PASS → docs/qa/gates/12.10-adopt-template-workspace.yml
Quality score: 90/100 (2 low-severity advisory items)

### Recommended Status

✓ Ready for Done — All acceptance criteria met, all tests passing, all prior findings resolved.
(Story owner decides final status)
