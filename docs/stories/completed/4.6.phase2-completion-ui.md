...
Superseded — replaced by Story 16.4 (single-phase generation, no Phase 2 needed)
        ...
        <div>
          <h3 className="text-sm font-semibold text-gray-900">
            {status.status === 'in_progress'
              ...
              ? '⏸️ Kompletterar din laglista, tar lite längre tid än förväntat...'
              : 'Kompletterar din laglista...'}
          </h3>
          <p className="text-xs text-gray-600">
            {status.progress}/{status.total} lagar
            {estimatedTime && estimatedTime > 0 && (
              <span className="ml-2">• ~{estimatedTime} sekunder kvar</span>
            )}
          </p>
        </div>
        <button
...
function showCompletionToast(totalLaws: number) {
  ...
  toast.success(
    `Klar! ${totalLaws} lagar i din lista är nu kompletta och aktiverade för ändringsbevakning`,
    {
      duration: 8000,
      position: 'top-center',
    }
      ...
      <div className="flex items-start">
        ...
        <div>
          <h3 className="text-sm font-semibold text-yellow-900">
            Vi slutför din laglista snart
          </h3>
          <p className="text-sm text-yellow-800 mt-1">
            Det tar lite längre tid än förväntat, men vi jobbar på det! Du får ett
            mejl när din laglista är komplett.
          </p>
          <p className="text-xs text-yellow-700 mt-2">
            Du kan fortsätta använda applikationen med de lagar som redan är
            genererade.
          </p>
        </div>
      </div>
