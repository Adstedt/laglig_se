...
### Post-QA Fixes (Drag-and-Drop Layout Issues)

- **Horizontal scroll fix:** Wrapped `DndContext` in `<div className="overflow-hidden">` to prevent page from scrolling horizontally when dragging cards near viewport edge
- **Drop target highlight:** Added `isDropTarget` prop to GroupAccordion that shows `border-primary bg-primary/5` when dragging a card over a different group
- **Layout stability fix:** Changed group border from `border` to `border-2 border-transparent` so border size is consistent - only color changes on drop target highlight (prevents layout shift)
...
| `components/features/document-list/grouped-document-list.tsx`      | Modified - Added overflow-hidden wrapper, drop target highlight, border fix        |
