...
| `app/actions/admin-impersonate.ts`                                   | NEW — startImpersonation(), endImpersonation() server actions. Sets/clears `admin_impersonating` marker cookie. Audit log before cookies.                           |
| `app/actions/admin-auth.ts`                                          | MODIFIED — Changed `admin_session` cookie path from `/admin` to `/` and sameSite from `strict` to `lax` (required for impersonation detection on workspace routes)  |
| `lib/admin/auth.ts`                                                  | MODIFIED — Added `ADMIN_IMPERSONATING_COOKIE`, `getNextAuthCookieName()`, `isImpersonating()`, `getImpersonationInfo()`. Uses marker cookie (not email comparison). |
| `components/admin/impersonation-banner.tsx`                          | NEW — Fixed banner component with loading state                                                                                                                     |
| `components/admin/impersonate-button.tsx`                            | NEW — Button + AlertDialog for user detail page                                                                                                                     |
| `app/(workspace)/layout.tsx`                                         | MODIFIED — Added ImpersonationBanner in both return paths                                                                                                           |
| `app/admin/(dashboard)/users/[id]/page.tsx`                          | MODIFIED — Replaced disabled button with ImpersonateButton, removed unused imports                                                                                  |
| `tests/unit/actions/admin-impersonate.test.ts`                       | NEW — 11 tests for server actions (includes marker cookie verification)                                                                                             |
| `tests/unit/lib/admin/impersonation-helpers.test.ts`                 | NEW — 11 tests for detection helpers (marker cookie based)                                                                                                          |

### Debug Log References

- `pnpm vitest run tests/unit/actions/admin-impersonate.test.ts tests/unit/lib/admin/impersonation-helpers.test.ts` — 22 tests pass (11 server actions + 11 helpers)
...
- **QA Fix: CODE-001-RESIDUAL** — Removed extra `adminEmail` prop from `ImpersonationBanner` in normal layout return path (TypeScript error TS2322)
- **Manual testing fix: admin_session cookie path** — Changed `admin_session` cookie path from `/admin` to `/` in `app/actions/admin-auth.ts` (both login and logout). The `/admin` path prevented the cookie from being read on workspace routes, breaking banner detection and `endImpersonation()`. Also changed `sameSite` from `strict` to `lax` for consistency.
- **Manual testing fix: marker cookie for impersonation detection** — Replaced email-comparison approach in `isImpersonating()` and `getImpersonationInfo()` with a dedicated `admin_impersonating` marker cookie. The email comparison assumed admin panel email matches the admin's NextAuth login email, which fails when they differ (e.g. admin logs into admin panel with one email, regular app with another). The marker cookie is set in `startImpersonation()` and cleared in `endImpersonation()` — explicit state tracking with no inference. Added `ADMIN_IMPERSONATING_COOKIE` constant to `lib/admin/auth.ts`.
- 22 automated tests now passing (11 server actions + 11 helpers)
...
| 2026-02-03 | 1.4     | Manual testing fixes: (1) Changed admin_session cookie path `/admin`→`/` so it's readable on workspace routes for banner/endImpersonation. (2) Replaced email-comparison impersonation detection with `admin_impersonating` marker cookie — email comparison failed when admin panel email differs from NextAuth login email. 22 tests passing. Manually verified end-to-end. | Quinn (QA)  |
