...
- [x] **Task 3: Create Type-Specific Tables** (AC: 3)
  - [ ] Add `CourtCase` model:
    ...
    - [ ] `courtName` (String) // "Högsta domstolen", "Hovrätten över Skåne och Blekinge"
...
- [x] **Task 5: Create `amendments` Table with Enhanced Metadata** (AC: 5)
  - [ ] Add `Amendment` model with 7 competitive-feature fields:
    ...
    - [ ] `amendingLawTitle` (String) // "Lag (2025:732) om ändring i arbetsrättslagstiftningen"
    - [ ] `publicationDate` (DateTime) // When amending law was published
    - [ ] `effectiveDate` (DateTime?) // When amendment takes effect (can be future)
    - [ ] `affectedSectionsRaw` (String?) // Notisum format: "ändr. 6 kap. 17 §; upph. 8 kap. 4 §"
...
- [x] **Task 6: Create `document_subjects` Table** (AC: 6)
  - [ ] Add `DocumentSubject` model:
    ...
    - [ ] `subjectName` (String) // "Arbetsrätt", "Dataskydd & GDPR"
    - [ ] `createdAt` (DateTime @default(now()))
    - [ ] Relation: `document LegalDocument @relation(fields: [documentId], references: [id])`
...
- [x] **Task 9: Insert Test Data** (AC: 10)
  ...
  - [ ] Insert sample amendment with all 7 fields:
    - [ ] Link Semesterlag → Amending law (mock data)
    - [ ] affectedSections: {amended: ["3:2"], repealed: []}
    - [ ] summary: "Ändring i beräkning av semesterersättning vid anställningens upphörande."
...
1. **Empty SFS numbers (133 entries)**: Historical documents from 1867-1958 (Riksbank/Riksgäldskontoret reglements) have empty `beteckning` fields. They appear as "SFS " with no number and cannot be properly indexed.
...
3. **Migration Workflow:**
   - Develop schema in `dev` → Test locally → `prisma migrate dev`
   - After QA approval → Deploy to `prod` → `prisma migrate deploy`
...
- Free tier sufficient for MVP (500MB → Pro at Month 6)
...
├─ LegalDocument (polymorphic table for all content types)
├─ CourtCase (type-specific: court cases)
├─ EuDocument (type-specific: EU legislation)
├─ CrossReference (document relationships)
└─ Amendment (SFS law change tracking)
```

**Related entities (NOT in this story, will be added later):**

- `DocumentSubject` (categorization) ← Story 2.6
- `LawVersionHistory` (change tracking) ← Story 2.11
- `LawEmbedding` (RAG chunks) ← Story 2.10
...
1. **amendingLawTitle:** Full title of amending law ("Lag (2025:732) om ändring i arbetsrättslagstiftningen")
2. **publicationDate:** When amending law was published in SFS
3. **effectiveDate:** When amendment takes effect (can be future date)
4. **affectedSectionsRaw:** Notisum-style string: "ändr. 6 kap. 17 §; upph. 8 kap. 4 §"
...
```json
{
  ...
  "amendingLawTitle": "Lag (2021:1112) om ändring i semesterlagen (1977:480)",
  "publicationDate": "2021-12-15",
  "effectiveDate": "2022-01-01",
  "affectedSectionsRaw": "ändr. 3 kap. 2 §; upph. 5 kap. 7 §",
  "affectedSections": {
    "amended": ["3:2"],
    "repealed": ["5:7"],
    "new": [],
  ...
  "summary": "Ändring i beräkning av semesterersättning vid anställningens upphörande. Semesterersättning ska nu beräknas på ny förordnad lön istället för tidigare lön. Paragrafen om fribelopp upphävs.",
  "summaryGeneratedBy": "GPT_4",
...
laglig_se/
├── prisma/
│   ├── schema.prisma         # UPDATE - Add 5 new models
│   ├── migrations/           # NEW - Migration files generated
│   └── seed-multi-content.ts # NEW - Test data seed script
├── lib/
│   └── db/
│       ├── prisma.ts         # EXISTING - Prisma client singleton
│       └── queries/          # FUTURE - Query functions (Story 2.2+)
...
// ✅ GOOD: Type-safe document creation
...
// ✅ GOOD: Type-safe query with IntelliSense
...
```typescript
// ✅ GOOD: Selective field fetching (performance)
...
describe('Multi-Content-Type Schema', () => {
  ...
  it('creates court case with relation', async () => {
    const doc = await prisma.legalDocument.create({
      data: {
        documentNumber: 'AD 2023 nr 45',
        title: 'Uppsägning p.g.a. arbetsbrist',
...
5. ✅ Table names use snake_case via `@@map()`
6. ✅ Test data seed script created
7. ✅ Migration tested in **dev** database first
8. ✅ Backup of production database taken

**After migration:**

1. ✅ Run `pnpm prisma generate` to update types
2. ✅ Run test suite to verify schema
3. ✅ Seed test data with `pnpm prisma db seed`
4. ✅ Query test data to verify relations work
...
- ✅ 5 enums: ContentType (9 values), DocumentStatus, SummaryGeneratedBy, AmendmentDetectedMethod, ReferenceType
- ✅ 9 tables: legal_documents, court_cases, eu_documents, cross_references, amendments, document_subjects, users, workspaces, workspace_members
...
1. ✅ ContentType enum with 9 values (including COURT_CASE_MOD, COURT_CASE_MIG)
2. ✅ legal_documents polymorphic table with search_vector and embedding fields
3. ✅ court_cases and eu_documents type-specific tables
4. ✅ cross_references table with bidirectional relations
5. ✅ amendments table with 7 enhanced fields (competitive feature)
6. ✅ document_subjects table for categorization
7. ✅ Prisma schema updated with all models, enums, relations
8. ✅ Migration generated and applied to dev database
9. ✅ TypeScript types generated (auto-generated by prisma db push)
10. ✅ Test data inserted and validated for all content types
...
- **Coding Standards**: ✓ PASS - Schema follows Prisma naming conventions (PascalCase models, camelCase fields, snake_case tables)
- **Project Structure**: ✓ PASS - Files in correct locations per `docs/architecture/12-unified-project-structure.md`
- **Testing Strategy**: ✗ FAIL - No automated tests despite story documentation showing test patterns (Story file lines 476-542)
- **All ACs Met**: ⚠ PARTIAL - 10/10 ACs technically met, but AC8 (prod database) intentionally deferred, AC10 (test data) not verified via tests
...
**Test Coverage**: ⚠️ No automated tests validate this requirement

**Acceptance Criteria Coverage:**

1. ✅ **AC1**: ContentType enum - 9 values present (`schema.prisma:116-126`)
2. ✅ **AC2**: legal_documents table - All 13 fields including search_vector, embedding (`schema.prisma:81-114`)
3. ✅ **AC3**: court_cases and eu_documents tables - Proper relations with CASCADE (`schema.prisma:161-187`)
4. ✅ **AC4**: cross_references table - Bidirectional relations configured (`schema.prisma:189-204`)
5. ✅ **AC5**: amendments table - All 7 competitive fields present (`schema.prisma:206-235`)
6. ✅ **AC6**: document_subjects table - Unique constraint configured (`schema.prisma:237-251`)
7. ✅ **AC7**: Prisma schema complete - All models, enums, indexes, relations
8. ⚠️ **AC8**: Migration applied to dev ✅, prod ❌ (intentionally deferred per dev notes)
9. ✅ **AC9**: TypeScript types - Auto-generated via Prisma client
10. ✅ **AC10**: Test data seed script created (`seed-multi-content.ts`)
...
- [x] Run test suite and ensure all tests pass: **18/18 tests passing** ✅
...
**Gate**: PASS ✅ → `docs/qa/gates/2.1-design-multi-content-type-data-model.yml`
...
- ✅ TEST-001: Created comprehensive integration test suite
- ✅ VAL-001: Verified seed data exists in database
- ✅ AC-001: User confirmed single-database strategy for now
...
- ✗ **AC10 Validation Failed**: Cannot verify test data insertion without running tests
- ✗ **Schema Validation Failed**: Cannot verify tables, indexes, and relations programmatically
- ✗ **Constraint Enforcement Failed**: Cannot verify unique constraints, foreign keys, CASCADE deletes
- ✗ **Type Safety Failed**: Cannot verify Prisma type generation beyond enum checks
...
**Gate**: FAIL ❌ → `docs/qa/gates/2.1-design-multi-content-type-data-model.yml` (UPDATED)
...
✅ **All issues previously identified are RESOLVED**:
...
**Gate**: PASS ✅ → `docs/qa/gates/2.1-design-multi-content-type-data-model.yml` (RESTORED)
...
- ✅ Excellent Prisma schema design (polymorphic pattern, proper indexes)
- ✅ Comprehensive test suite (18 test cases covering all ACs)
- ✅ Test environment properly configured and documented
- ✅ Security: Parameterized queries, secure credential management
- ✅ Performance: Comprehensive indexes, connection pooling
- ✅ Documentation: database-setup.md, database-testing.md, test README
