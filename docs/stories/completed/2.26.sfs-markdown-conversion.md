...
**Superseded** (2026-04-13) — delivered by a different architectural path than originally proposed.

The premise of this story was that PDF text extraction produces artifacts (embedded page numbers, broken sentences, misplaced `§` markers) that need a regex cleanup library to convert into well-structured markdown. The codebase has since moved to a **PDF-direct LLM pipeline** that makes this cleanup unnecessary:

- `lib/sfs/amendment-llm-prompt.ts` — sends the PDF buffer to Claude as `type: "document"` and returns semantic HTML
- `lib/transforms/html-to-markdown.ts` — derives clean markdown from the HTML
- `lib/transforms/html-to-json.ts` — derives structured JSON from the same HTML
- `lib/sfs/llm-output-validator.ts` — validates the output

Because the LLM reads the PDF directly, there is no text-extraction step to clean up. The `§` markers, headings, and structure are produced correctly by the LLM and preserved through the HTML → markdown transform. Markdown for RAG is already being generated via this path.

This story is preserved as historical context. If a gap in the PDF-direct pipeline's markdown quality ever appears, open a new, narrower story rather than resurrecting this one.
