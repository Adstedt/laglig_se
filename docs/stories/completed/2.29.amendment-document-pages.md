...
- [ ] **Phase 2: Amendment Processing (~35k documents)**
  ...
  - [x] Review & refine markdown/JSON transforms based on test batch output
  - [x] Submit production batch for all amendment PDFs (~$1,400 via Anthropic Batch API)
  - [x] Monitor batch completion and download results (~24h turnaround)
  - [x] Process results and verify quality
  - [x] Ensure `Amendment.amending_document_id` is populated for all entries
  - [x] Quality review: Sample 50-100 outputs for accuracy

- [x] **Phase 3: Backfill Existing Documents**
  - [x] Base Laws: Generate `markdown_content` and `json_content` from existing `html_content`
  - [ ] Court Cases: Same backfill process
  - [ ] EU Documents: Same backfill process

- [x] **Phase 4: E2E Testing**
  - [x] E2E test: Navigate from base law → history → amendment page → back to base law
  - [x] E2E test: Search for amendment by SFS number, click through to page
  - [x] E2E test: Filter catalogue by amendments
  - [x] E2E test: Verify HTML content renders correctly on amendment pages
