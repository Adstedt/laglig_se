...
Superseded by Story 6.11 (notification system implemented all core preference toggles including law change types, preferences UI, and server actions). Remaining minor items (master toggle, warning dialogs, email unsubscribe links) deferred to backlog.
