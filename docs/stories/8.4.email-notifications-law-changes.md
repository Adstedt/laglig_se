...
2. Email structure includes:
   ...
   - CTA button "Review Changes" → Links to Changes tab
3. Priority badges (🟢/🟡/🔴) for each change
4. Dual links for each change:
   - [View on Laglig.se] → Opens diff view
   - [Official Riksdagen PDF] → Opens official source
5. Multiple law lists → Send separate email per list (workspace-scoped)
6. React Email for templates with branded design
7. Unsubscribe link in footer
8. Track email opens using UTM parameters
9. Email sent only if ≥1 change detected in last 24 hours
10. From address: "Laglig.se <notifications@laglig.se>"
11. Subject line: "Lagändringar: [N] nya ändringar i din laglista"
        ...
        <Container style={{ maxWidth: '600px', margin: '0 auto', backgroundColor: '#ffffff', padding: '0' }}>
          ...
          <Section style={{ padding: '24px' }}>
            ...
            <Text style={{ fontSize: '14px', color: '#6b7280', margin: '0' }}>
              Följande lagar i din lista <strong>{workspaceName}</strong> har ändrats de senaste 24 timmarna:
            </Text>
          </Section>
          ...
          <Section style={{ padding: '16px 24px', fontSize: '12px', color: '#6b7280', textAlign: 'center' }}>
            <Text style={{ margin: '0 0 8px 0' }}>
              Detta är en automatisk notifiering från Laglig.se
            </Text>
            <Link href={unsubscribeUrl} style={{ color: '#3b82f6', textDecoration: 'none' }}>
              Avbeställ daglig sammanfattning
            </Link>
          </Section>
        </Container>
      </Body>
    ...
    for (const workspace of workspaces) {
      ...
      const changes = workspace.workspaceLaws.flatMap((wl) =>
        wl.law.lawChanges.map((change) => ({
          ...
          aiSummary: change.aiSummary || 'Ändring upptäckt. Se detaljer för mer information.',
          ...
          officialUrl: change.officialSourceUrl || `https://www.riksdagen.se/sv/dokument-och-lagar/dokument/svensk-forfattningssamling/${wl.law.sfsNummer}`
        }))
      )
      ...
      if (preferences && !preferences.emailDailyDigest) {
        console.log(`Skipping email for ${workspace.owner.email} - disabled in preferences`)
        continue
      }
      ...
      try {
        await sendEmail({
          ...
          subject: `Lagändringar: ${changes.length} nya ändringar i din laglista`,
        ...
        console.error(`Failed to send email to ${workspace.owner.email}:`, error)
        emailsFailed++
      }
    }
...
export async function POST(request: Request) {
  return withAuth(async ({ userId }) => {
    ...
    if (unsubscribe === 'daily_digest') {
      ...
      return NextResponse.json({ success: true, message: 'Daily digest unsubscribed' })
    }
