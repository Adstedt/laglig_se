...
function evaluateRetrieval(retrievedChunks: string[], relevantChunks: string[], k: number) {
...
const strategies = [
  ...
  { name: 'structural-section', type: 'structural', delimiter: '§', maxTokens: 800 },
...
async function runExperiment() {
  ...
  for (const strategy of strategies) {
    ...
    for (const query of testQueries) {
      ...
      const metrics = evaluateRetrieval(retrieved, query.relevantChunks, k=10)
