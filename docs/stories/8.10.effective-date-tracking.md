...
export async function extractEffectiveDateAI(
  newContent: string,
  lawTitle: string
): Promise<{ effectiveDate: Date | null; confidence: 'high' | 'medium' | 'low' }> {
  try {
...
Hitta ikraftträdandedatumet. Svara med YYYY-MM-DD eller "UNKNOWN".`
    ...
    if (result && result !== 'UNKNOWN') {
      ...
      if (dateMatch) {
        const date = new Date(dateMatch[1], parseInt(dateMatch[2]) - 1, parseInt(dateMatch[3]))
...
async function detectAndStoreChange(law: any, oldContent: string, newContent: string) {
  ...
  console.log(`Change detected for ${law.sfsNummer}, effective: ${effectiveDate || 'unknown'}`)
}
```
