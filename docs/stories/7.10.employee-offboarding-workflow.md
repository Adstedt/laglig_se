...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  return withWorkspace(async ({ workspaceId, userId, role }) => {
    ...
    if (employee.status === 'inactive') {
      return NextResponse.json({ error: 'Employee already inactive' }, { status: 400 })
    }
  ...
  return withWorkspace(async ({ workspaceId }) => {
    ...
    const exportData = {
      ...
      employment: {
        ...
        manager: employee.manager ? { name: employee.manager.name, email: employee.manager.email } : null
      },
      compliance: {
