...
Ready for Review (Production Fix Applied)
...
**Production Fix Applied (2025-12-10):**

- **Root Cause:** `router.prefetch()` in Next.js 15+ only performs a "segment prefetch" (route tree metadata) for dynamic routes, NOT a full data prefetch. This was confirmed via Chrome DevTools MCP inspection showing:
  - Prefetch requests had header `next-router-segment-prefetch:/_tree` (tree-only)
  - Navigation requests had full `next-router-state-tree` with `"refetch"` flag
  - Response `cache-control: no-cache, no-store` prevented caching
- **Fix:** Added `prefetch={true}` to `<Link>` component in `catalogue-result-card.tsx`
  - This forces Next.js to do a **full prefetch** including page data, not just route tree
  - Per Next.js docs: `prefetch={true}` = "full route will be prefetched for both static and dynamic routes"
- **Note:** The custom `prefetchManager` with `router.prefetch()` is now redundant for catalogue cards since `<Link prefetch={true}>` handles it automatically. However, it's retained for:
  - Related docs prefetching on detail pages (where links may not be in viewport)
  - Programmatic prefetch scenarios
...
| 2025-12-10 | 1.5     | **Production Fix** - Added `prefetch={true}` to Link component to force full prefetch for dynamic routes                                                              | James (Dev) |
