...
export async function suggestRelevantLaws(
  employee: any
): Promise<Array<{ lawId: string; reason: string }>> {
...
[
  ...
  for (const suggestion of suggestions) {
    const workspaceLaw = workspaceLaws.find(
      (wl) => wl.law.sfsNummer === suggestion.sfsNummer
    )
    if (workspaceLaw) {
      result.push({
...
export async function autoAssignLawsToEmployee(employeeId: string) {
  ...
  console.log(
    `Created ${suggestions.length} law suggestions for employee ${employee.id}`
  )
}
```
