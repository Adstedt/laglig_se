...
export async function GET(request: Request) {
  return withWorkspace(async ({ workspaceId }) => {
    ...
    if (columns.length === 0) {
      const defaults = [
        { columnId: 'not_started', label: 'Not Started', color: 'gray', position: 0, isDefault: true },
        { columnId: 'in_progress', label: 'In Progress', color: 'blue', position: 1, isDefault: true },
        { columnId: 'blocked', label: 'Blocked', color: 'red', position: 2, isDefault: true },
        { columnId: 'review', label: 'Review', color: 'yellow', position: 3, isDefault: true },
        { columnId: 'compliant', label: 'Compliant', color: 'green', position: 4, isDefault: true }
      ]
...
export async function PUT(request: Request) {
  return withWorkspace(async ({ workspaceId, role }) => {
    ...
    const deletedColumns = existingColumns.filter((c) => !newColumnIds.includes(c.id))
