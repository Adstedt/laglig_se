...
export async function POST(request: Request) {
  return withWorkspace(async ({ workspaceId, userId, role }) => {
    ...
    const { data, columnMapping, dateFormat, skipInvalid } = await request.json()
      ...
      try {
        ...
        if (!employmentDate) {
          ...
          if (skipInvalid) {
            ...
            return NextResponse.json({ error: `Row ${rowNumber}: ${error}` }, { status: 400 })
          }
        }
        ...
        const normalizedContractType = normalizeContractType(mapped.contractType)
...
Only respond with the normalized role, nothing else.`
    ...
    const normalized = response.choices[0].message.content?.trim().toLowerCase() || 'employee'
    return normalized
  } catch (error) {
    ...
    if (lower.includes('intern') || lower.includes('praktikant')) return 'intern'
    if (lower.includes('consultant') || lower.includes('konsult')) return 'consultant'
    if (lower.includes('build') || lower.includes('bygg')) return 'construction_worker'
    return 'employee'
  }
}
...
function normalizeContractType(input: string): string {
  ...
  if (lower.includes('permanent') || lower.includes('tillsvidare')) return 'permanent'
