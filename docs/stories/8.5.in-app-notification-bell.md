...
6. Click on change → Opens Changes tab, scrolls to that change
...
export async function GET(request: Request) {
  return withWorkspace(async ({ workspaceId }) => {
    ...
    const highPriorityCount =
      priorityCounts.find((p) => p.priority === 'high')?._count || 0
    const mediumPriorityCount =
      priorityCounts.find((p) => p.priority === 'medium')?._count || 0
          ...
          <div className="max-h-96 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="px-4 py-8 text-center">
                ...
                <p className="text-sm text-gray-600 mt-2">No new changes ✅</p>
              </div>
            ) : (
