  ...
  if (changes.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-lg shadow">
        ...
        <p className="mt-4 text-lg font-medium text-gray-900">No unacknowledged changes ✅</p>
        <p className="text-sm text-gray-600 mt-1">
...
const PRIORITY_CONFIG = {
  high: { color: 'bg-red-100 text-red-800', icon: '🔴', label: 'High Priority' },
  medium: { color: 'bg-yellow-100 text-yellow-800', icon: '🟡', label: 'Medium Priority' },
  low: { color: 'bg-green-100 text-green-800', icon: '🟢', label: 'Low Priority' }
}
...
- Changes sorted by priority (High → Medium → Low), then date
