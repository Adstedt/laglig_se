 ...
 * The accordions are chunked into collapsible groups (Efterlevnad /
 * Granskning) using the law modal's `AccordionGroupLabel` divider + Safiro
 * heading font, so the cycle modal reads as a sibling of the revamped law
 * list modal. All sections default to open so auditors see everything at a
 * glance; group/section state is NOT persisted (unlike the law modal, which
 * is localStorage-backed) because the cycle audit flow is a focused review
 * session, not exploratory browsing.
 *
...
import { AccordionGroupLabel } from '@/components/features/document-list/legal-document-modal/accordion-group-label'
...
// Collapsible group ids. Mirrors the law modal's group-divider chunking, but
// the cycle modal only needs two groups: the team's compliance position
// (Efterlevnad) and the auditor's review material + output (Granskning).
type GroupId = 'efterlevnad' | 'granskning'
...
export function CycleItemModalLeftPanel({
  ...
  // Collapsible group state. Ephemeral (not localStorage-backed) — the audit
  // flow is a focused session, so groups reset to open each time the modal
  // opens. A group id present here = folded.
    ...
    <ScrollArea className="h-full max-h-[calc(90vh-60px)] max-md:max-h-none">
      <div className="space-y-4 p-6">
        ...
        <header className="space-y-1">
          <h2 className="font-safiro text-xl font-medium leading-tight tracking-[-0.01em] text-foreground">
            {item.lawTitle}
          </h2>
        ...
        <Accordion type="multiple" defaultValue={[...ACCORDION_ITEMS]}>
          {/* ===== Efterlevnad — the team's compliance position ===== */}
          <Collapsible
                ...
                <AccordionContent className="px-4 pb-4">
                  <KravpunkterSnapshotList
                    ...
                    itemResponsibleUserId={
                      item.sourceResponsibleUser?.id ?? null
                    }
                    {...(onSuggestFindingForRequirement
                      ? {
                          onSuggestFindingForRequirement,
          ...
          {/* ===== Granskning — evidence reviewed + findings recorded ===== */}
          <Collapsible
            open={!collapsedGroups.includes('granskning')}
            onOpenChange={() => toggleGroup('granskning')}
            className="mt-4"
          >
            <CollapsibleTrigger asChild>
              <AccordionGroupLabel>Granskning</AccordionGroupLabel>
            </CollapsibleTrigger>
            <CollapsibleContent className="space-y-2 pt-2">
              {/* ------- Underlag ------- */}
              ...
              >
                <AccordionTrigger className="rounded-t-lg px-4 py-3 hover:bg-muted/50 hover:no-underline data-[state=closed]:rounded-lg">
                  <div className="flex items-center gap-2 text-base font-semibold text-foreground">
                    ...
                    <span className="text-xs font-normal text-muted-foreground">
                      {openFindings.length} öppna · {closedFindings.length}{' '}
                      stängda
                    </span>
                  </div>
                </AccordionTrigger>
