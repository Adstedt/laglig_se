...
import { promoteTaskFilesToListItems } from '@/app/actions/tasks'
import type { TaskPriority } from '@prisma/client'
import type { WorkspaceMember } from '../task-workspace'
import type { TaskColumnWithCount } from '@/app/actions/tasks'
import { toast } from 'sonner'
import { TaskCompletionConfirmDialog } from './completion-confirm-dialog'
...
export function DetailsBox({
  ...
  // Story 6.7c: completion-confirmation gating for file→list-item promotion.
  // When a user transitions a task from not-done → done AND the task has both
  // files and linked law list items, defer the actual status change until the
  // user confirms which (file × list-item) pairs to promote to first-class
  // direct links. See _prototypes/task-completion-link-files-flow.html.
  const [pendingDoneColumnId, setPendingDoneColumnId] = useState<string | null>(
    null
  )
  const [isPromoting, setIsPromoting] = useState(false)

  // Filter out folders (Story 6.7b) — only promote actual files.
  const realFiles = task.evidence.filter((e) => e.original_filename !== null)

  const performStatusChange = async (columnId: string) => {
  ...
  const handleStatusChange = async (columnId: string) => {
    ...
    const wasDone = task.column.is_done
    const willBeDone = targetColumn?.is_done ?? false
    const shouldPrompt =
      !wasDone &&
      willBeDone &&
      realFiles.length > 0 &&
    ...
    if (shouldPrompt) {
      setPendingDoneColumnId(columnId)
      return
    }

    await performStatusChange(columnId)
  }

  const handleConfirmCompletion = async (
    pairs: Array<{ fileId: string; listItemId: string }>
  ) => {
    const columnId = pendingDoneColumnId
    if (!columnId) return

    setIsPromoting(true)
    try {
      if (pairs.length > 0) {
        const linkResult = await promoteTaskFilesToListItems(pairs)
        if (!linkResult.success) {
          // Surface the failure but don't block the status transition — the
          // user's primary intent was to complete the task.
          toast.error('Kunde inte länka bilagor', {
            description: linkResult.error,
          })
        }
      }
      const ok = await performStatusChange(columnId)
      setPendingDoneColumnId(null)
      if (ok) {
        if (pairs.length > 0) {
          toast.success(
            `Uppgift klar — ${pairs.length} bilag${pairs.length === 1 ? 'a' : 'or'} direktlänkad${pairs.length === 1 ? '' : 'e'}`
          )
        } else {
  ...
  return (
    <>
      ...
      <TaskCompletionConfirmDialog
        open={pendingDoneColumnId !== null}
        onOpenChange={(open) => {
          if (!open && !isPromoting) setPendingDoneColumnId(null)
        }}
        taskTitle={task.title}
        files={realFiles.map((e) => ({
          id: e.file_id,
          filename: e.original_filename ?? e.filename,
          size: e.file_size,
        }))}
        listItems={task.list_item_links.map((l) => ({
          id: l.law_list_item.id,
          documentTitle: l.law_list_item.document.title,
          documentNumber: l.law_list_item.document.document_number,
          lawListName: l.law_list_item.law_list.name,
        }))}
