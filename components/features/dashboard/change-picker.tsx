  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[624px] gap-0 overflow-hidden p-0">
        ...
        {/* Wrapper so the gradient overlay can absolute-position over the
            scroll container without affecting layout. */}
        <div className="relative border-t">
          {/* max-h is tuned so the last visible row is partially cut on a
              full list (~5.7 rows visible) — the row peek is the strongest
              "there's more below" affordance. */}
          ...
          {/* Scroll fade: signals "more below" via a soft gradient that
              dissolves the last visible row into the background. Only when
              the list is likely to overflow (>5 items), so short lists
              don't get a stray fade on their final row. */}
          {itemCount > 5 && (
            <div
              className="pointer-events-none absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-background via-background/75 to-transparent"
              aria-hidden="true"
            />
          )}
        </div>

        {/* Footer count: replaces the title-row pill. Provides a stable
            visual end-of-list marker that pairs with the scroll fade —
            gradient hints "scroll up", footer says "this many total". */}
        {itemCount > 0 && (
          <div className="border-t bg-muted/30 px-6 py-2.5 text-xs text-muted-foreground">
            <span className="tabular-nums">
