...
function sparqlQuery(query) {
  return new Promise((resolve, reject) => {
    ...
    https
      .get(url, { headers: { 'User-Agent': 'Laglig.se/1.0' } }, (res) => {
        let data = ''
...
async function main() {
  ...
  const resourceTypes = [
    ...
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/REG_IMPL',
      name: 'Implementing Reg (REG_IMPL)',
      celex: 'R',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/REG_DEL',
      name: 'Delegated Reg (REG_DEL)',
      celex: 'R',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/DIR',
      name: 'Directive (DIR)',
      celex: 'L',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/DIR_IMPL',
      name: 'Implementing Dir (DIR_IMPL)',
      celex: 'L',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/DEC',
      name: 'Decision (DEC)',
      celex: 'D',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/DEC_IMPL',
      name: 'Implementing Dec (DEC_IMPL)',
      celex: 'D',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/RECO',
      name: 'Recommendation (RECO)',
      celex: 'H',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/OPIN',
      name: 'Opinion (OPIN)',
      celex: 'Q',
    },
    {
      uri: 'http://publications.europa.eu/resource/authority/resource-type/AGREE_INTERNATION',
      name: 'Intl Agreement',
      celex: 'A',
    },
  ]
  ...
  const allTypes = new Set([
    ...Object.keys(apiCounts),
    ...Object.keys(bulkCounts),
  ])
  ...
  for (const type of [...allTypes].sort()) {
    ...
    const sign = diff > 0 ? '+' : ''
    console.log(
      `  ${type}  |  ${api.toLocaleString().padStart(8)} |  ${bulk.toLocaleString().padStart(8)} | ${sign}${diff.toLocaleString()}`
    )
  }

  console.log('-----|-----------|------------|----------')
  console.log(
    `TOTAL|  ${apiTotal.toLocaleString().padStart(8)} |  ${bulkTotal.toLocaleString().padStart(8)} | ${apiTotal > bulkTotal ? '+' : ''}${(apiTotal - bulkTotal).toLocaleString()}`
  )
}
