...
async function main() {
  ...
  const sfsMatches = sfs.html_content.match(/<a[^>]*class=.paragraf.[^>]*>[\s\S]*?<\/a>/g) || []
  ...
  const afs = await p.legalDocument.findFirst({
    where: { content_type: 'AGENCY_REGULATION', html_content: { not: null }, full_text: { not: null } },
    select: { document_number: true, html_content: true },
  })
  const afsMatches = afs.html_content.match(/<a[^>]*class=.paragraf.[^>]*>[\s\S]*?<\/a>/g) || []
  ...
  if (amd) {
    const amdMatches = amd.html_content.match(/<a[^>]*class=.paragraf.[^>]*>[\s\S]*?<\/a>/g) || []
