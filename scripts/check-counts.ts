...
async function main() {
  ...
  const amendments = await prisma.legalDocument.count({ where: { content_type: 'SFS_AMENDMENT' } })
  const withHtml = await prisma.legalDocument.count({ where: { content_type: 'SFS_AMENDMENT', html_content: { not: null } } })
