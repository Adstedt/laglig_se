...
async function testSingleDocument(filename: string) {
  ...
  console.log('\n📊 REGEX PARSE:')
  console.log(
    `   Base law: ${regexResult.baseLaw?.name} (${regexResult.baseLaw?.sfsNumber})`
  )
  ...
  console.log(
    `   Base law: ${llmResult.baseLaw.name} (${llmResult.baseLaw.sfsNumber})`
  )
  ...
  if (llmResult.transitionalProvisions?.length) {
    console.log(
      `   Transitional: ${llmResult.transitionalProvisions.length} provisions`
    )
  }
  ...
  console.log(
    `   Diff:  +${llmResult.affectedSections.length - regexResult.affectedSections.length} more from LLM`
  )
...
async function main() {
  ...
  if (args[0] === '--all') {
    ...
    results.forEach((r) => {
      console.log(
        `| ${r.filename} | ${r.regexSections} | ${r.llmSections} | ${(r.confidence * 100).toFixed(0)}% |`
      )
