...
async function testCacheWarming() {
  ...
  try {
    ...
    if (visitCount > 0) {
      ...
      topVisited.forEach(v => {
        console.log(`   - ${v.document.document_number}: ${v.visit_count} visits`)
      })
    }
  } catch (error) {
  ...
  try {
    ...
    for (const doc of popularDocs.slice(0, 5)) {
      ...
      const icon = doc.source === 'both' ? '🏆' : 
                   doc.source === 'law_lists' ? '📋' : '🌐'
      console.log(`   ${icon} ${document?.document_number || 'Unknown'} (score: ${doc.score})`)
    }
  } catch (error) {
