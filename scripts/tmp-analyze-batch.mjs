...
const files = fs.readdirSync(dir).filter(f => f.endsWith('.json') && !f.includes('raw'))
...
for (const file of files.sort()) {
  ...
  const docNumPrefix = json.documentNumber?.startsWith('SFS') ? 'SFS' : 'missing-SFS'

  console.log(`--- ${file} ---`)
  console.log(`  Title: ${json.title}`)
  console.log(`  DocNumber: ${json.documentNumber} [${docNumPrefix}]`)
  console.log(`  Chapters: ${allChapters.length}, Sections: ${totalSections}, Paragraphs: ${totalParagraphs}`)
  console.log(`  Roles used: ${[...roles].sort().join(', ')}`)
  console.log(`  Numbered list items as paragraphs: ${numberedListIssues}`)
  console.log(`  Transition provisions: ${json.transitionProvisions?.length || 0}`)
  ...
  for (const ch of allChapters) {
    for (const sec of ch.sections || []) {
      for (const p of sec.paragraphs || []) {
        if (/^(Lag|Förordning)\s+\(\d{4}:\d+\)\s*\.?\s*$/.test(p.text.trim())) leakedRefs++
      }
    }
  }
