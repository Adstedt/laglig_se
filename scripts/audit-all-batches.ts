...
const ALL_BATCH_IDS = [
  // Original parts (completed earlier with credit errors)
...
async function main() {
  ...
  console.log('Auditing all batches...\n')
  console.log(
    'ID           | Status    | Succeeded | Errored | Canceled | Total'
  )
  console.log(
    '-------------|-----------|-----------|---------|----------|------'
  )
  ...
  for (const batchId of ALL_BATCH_IDS) {
    try {
      ...
      const status = batch.processing_status.padEnd(9)

      console.log(
        `${shortId} | ${status} | ${String(c.succeeded).padStart(9)} | ${String(c.errored).padStart(7)} | ${String(c.canceled).padStart(8)} | ${c.succeeded + c.errored + c.canceled + c.processing}`
      )

      totalRequests += c.succeeded + c.errored + c.canceled + c.processing
      totalSucceeded += c.succeeded
      totalErrored += c.errored
      totalCanceled += c.canceled
      totalExpired += c.expired
    } catch (e: any) {
      console.log(`${batchId.slice(-8)} | ERROR: ${e.message}`)
    }
  }
  ...
  console.log(
    `TOTAL        |           | ${String(totalSucceeded).padStart(9)} | ${String(totalErrored).padStart(7)} | ${String(totalCanceled).padStart(8)} | ${totalRequests}`
  )
  ...
  console.log(`\nNeed to reprocess: ${totalErrored + totalCanceled} requests`)
}
