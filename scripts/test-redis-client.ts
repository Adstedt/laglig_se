...
async function testRedisClient() {
  ...
  const testValue = { message: 'Hello from client', timestamp: new Date().toISOString() }
  ...
  // Parse if string
  const parsed = typeof getRawResult === 'string' ? JSON.parse(getRawResult) : getRawResult
  ...
  if (existingItem) {
    const itemParsed = typeof existingItem === 'string' ? JSON.parse(existingItem) : existingItem
