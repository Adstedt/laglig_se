...
async function main() {
  ...
  changes.forEach(c => {
    const loc = c.chapter ? `${c.chapter} kap. ${c.section} §` : `${c.section} §`
    console.log(c.amendment.sfs_number, '|', loc, '|', c.change_type)
    console.log('  new_text:', c.new_text ? c.new_text.substring(0, 100) + '...' : 'NULL')
    console.log('  old_text:', c.old_text ? c.old_text.substring(0, 100) + '...' : 'NULL')
    console.log()
  })

  // Count how many have text
  const withNewText = await prisma.sectionChange.count({ where: { new_text: { not: null } } })
  const withOldText = await prisma.sectionChange.count({ where: { old_text: { not: null } } })
