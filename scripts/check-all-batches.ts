...
async function main() {
  ...
  console.log('Checking all batch statuses...\n')
  ...
  for (const batchId of BATCH_IDS) {
    try {
      ...
      if (status === 'ended') {
        completedBatches++
        if (errored > 0) {
          console.log(`✓ ${batchId.slice(-8)}: ${succeeded} ok, ${errored} err`)
        } else {
          console.log(`✓ ${batchId.slice(-8)}: ${succeeded} ok`)
        }
      } else {
        inProgressBatches++
        console.log(`⏳ ${batchId.slice(-8)}: ${processing} processing`)
      }
    } catch (e: any) {
  ...
  console.log(`In progress: ${inProgressBatches}`)
