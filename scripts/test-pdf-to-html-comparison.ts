...
async function main() {
  ...
  for (const pdfFile of testPdfs) {
    ...
    if (opusResult.result.success) {
      ...
      console.log(
        `    ✓ ${opusResult.result.inputTokens} in / ${opusResult.result.outputTokens} out (${opusResult.result.durationMs}ms)`
      )
    } else {
    ...
    if (sonnetResult.result.success) {
      ...
      console.log(
        `    ✓ ${sonnetResult.result.inputTokens} in / ${sonnetResult.result.outputTokens} out (${sonnetResult.result.durationMs}ms)`
      )
    } else {
  ...
  const opusCost =
    (opusTotals.inputTokens * 15 + opusTotals.outputTokens * 75) / 1_000_000
  const sonnetCost =
    (sonnetTotals.inputTokens * 3 + sonnetTotals.outputTokens * 15) / 1_000_000

  console.log('Model     | Input Tok | Output Tok | Duration | Est. Cost')
  console.log('----------|-----------|------------|----------|----------')
  console.log(
    `Opus      | ${opusTotals.inputTokens.toLocaleString().padStart(9)} | ${opusTotals.outputTokens.toLocaleString().padStart(10)} | ${(opusTotals.duration / 1000).toFixed(1).padStart(6)}s | $${opusCost.toFixed(2)}`
  )
  console.log(
    `Sonnet    | ${sonnetTotals.inputTokens.toLocaleString().padStart(9)} | ${sonnetTotals.outputTokens.toLocaleString().padStart(10)} | ${(sonnetTotals.duration / 1000).toFixed(1).padStart(6)}s | $${sonnetCost.toFixed(2)}`
  )
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { results, opusTotals, sonnetTotals, opusCost, sonnetCost },
      null,
      2
    )
  )
