...
const EXPECTED_CHAPTERS = [
  ...
  {
    num: 6,
    title: 'Gemensamma bestämmelser om byggnads- och anläggningsarbete',
  },
...
function analyzeChapters(text) {
  ...
  const kapPatterns = [/(\d+)\s*kap\./gi, /Kapitel\s+(\d+)/gi]
...
function analyzeParagraphs(text) {
  ...
  // Get unique § numbers
  const uniqueNums = [...new Set(allParagraphs.map((p) => p.num))].sort(
    (a, b) => a - b
  )
...
async function run() {
  ...
  const foundNums = Object.keys(chaptersFound)
    .map(Number)
    .sort((a, b) => a - b)

  console.log(
    `  Expected chapters: ${EXPECTED_CHAPTERS.length} (kap. 1–${EXPECTED_CHAPTERS.length})`
  )
  console.log(`  Found chapter refs: ${foundNums.join(', ') || 'NONE'}`)

  const missingChapters = EXPECTED_CHAPTERS.filter(
    (ch) => !foundNums.includes(ch.num)
  )
  if (missingChapters.length > 0) {
    console.log(
      `  MISSING chapters: ${missingChapters.map((ch) => `kap. ${ch.num} (${ch.title})`).join(', ')}`
    )
  } else {
  ...
  if (pStats.gaps.length > 0 && pStats.gaps.length <= 20) {
    ...
    console.log(
      `  Missing § numbers: ${pStats.gaps.length} gaps (expected — § numbering restarts per chapter)`
    )
  }
  console.log()
  ...
  if (hasAllChapters && hasContent && hasParagraphs) {
    ...
    if (!hasContent)
      console.log(`  - Content too short (${(text || html).length} chars)`)
    if (!hasParagraphs)
      console.log(`  - Too few § references (${pStats.uniqueCount})`)
  }
