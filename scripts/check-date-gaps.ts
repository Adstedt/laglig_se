...
async function checkDateGaps() {
  ...
  try {
    ...
    let missingCount = 0
    const missingDocs: Array<{sfsNumber: string; title: string; publicerad: string; datum: string}> = []
    ...
    console.log(`  - Total checked: ${documents.filter(d => d.beteckning).length}`)
    console.log(`  - In database: ${documents.filter(d => d.beteckning).length - missingCount}`)
    console.log(`  - Missing: ${missingCount}`)
    console.log(`  - Coverage: ${((1 - missingCount / documents.filter(d => d.beteckning).length) * 100).toFixed(1)}%\n`)
    ...
    if (missingDocs.length > 0) {
      ...
      // Check date pattern
      console.log('\n📅 Date Analysis:')
      const sortedByDate = missingDocs.sort((a, b) => 
        new Date(b.publicerad).getTime() - new Date(a.publicerad).getTime()
      )
      
      if (sortedByDate.length > 0) {
        const newest = new Date(sortedByDate[0].publicerad)
        const oldest = new Date(sortedByDate[sortedByDate.length - 1].publicerad)
        
        console.log(`  Newest missing: ${sortedByDate[0].publicerad}`)
        console.log(`  Oldest missing: ${sortedByDate[sortedByDate.length - 1].publicerad}`)
        console.log(`  Date range: ${Math.ceil((newest.getTime() - oldest.getTime()) / (1000 * 60 * 60 * 24))} days`)
      }
    } else {
    ...
    recentDocs.forEach(doc => {
      console.log(`  - ${doc.document_number} (added ${doc.created_at.toISOString()})`)
    })
