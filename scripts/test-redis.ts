...
async function testRedis() {
  ...
  if (!isRedisConfigured()) {
    console.error('❌ Redis is not configured. Check your environment variables.')
    process.exit(1)
  }
  ...
  try {
    ...
    const testValue = { message: 'Hello from Laglig.se', timestamp: new Date().toISOString() }
    ...
    console.log(`   ✓ Hit rate: ${hits && misses ? Math.round((Number(hits) / (Number(hits) + Number(misses))) * 100) : 0}%`)
