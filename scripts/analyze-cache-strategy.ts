...
async function analyzeCacheStrategy() {
  ...
  try {
    ...
    const docCounts = uniqueDocs.map(d => d._count.document_id).filter(c => c > 0)
    ...
    for (const cacheSize of cacheSizes) {
      ...
      const coveredItems = topDocs.reduce((sum, doc) => sum + doc._count.document_id, 0)
      const coverage = (coveredItems / totalListItems) * 100
      
      console.log(`   Top ${String(cacheSize).padStart(3)} docs → ${String(coveredItems).padStart(5)} items (${coverage.toFixed(1)}% coverage)`)
    ...
    for (const item of topTen) {
      ...
      if (doc) {
        ...
        console.log(`   ${doc.document_number.padEnd(15)} - ${item._count.document_id} lists (${sizeKB} KB)`)
        if (doc.title) {
    ...
    const totalSizeBytes = documents.reduce((sum, doc) => sum + (doc.html_content?.length || 0), 0)
    const totalSizeMB = (totalSizeBytes / 1024 / 1024).toFixed(2)
    
    console.log('\n💾 Cache Size Estimate:')
    console.log(`   Top 200 documents total size: ${totalSizeMB} MB`)
    console.log(`   Average document size: ${(totalSizeBytes / documents.length / 1024).toFixed(1)} KB`)
    console.log(`   Redis cost estimate: ~$${(parseFloat(totalSizeMB) * 0.20 / 1000).toFixed(2)}/month`)
    ...
    console.log(`   Time to warm 200 docs: ~${estimatedTime.toFixed(1)} seconds`)
    console.log(`   With parallel batches of ${batchSize}: ~${(estimatedTime / 2).toFixed(1)} seconds`)
