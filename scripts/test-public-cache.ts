import { getCachedDocument, getCachedDocumentBySlug } from '@/lib/services/document-cache'
...
async function testPublicPageCache() {
  ...
  if (cachedId) {
    ...
    console.log('❌ Slug mapping NOT found - document was never cached via public page')
  ...
  const docKeys = keys.filter(k => !k.includes(':slug:') && !k.includes(':number:') && !k.includes(':content:'))
