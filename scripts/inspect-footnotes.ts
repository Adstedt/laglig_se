...
async function main() {
  ...
  const allFootnotes: Array<{ doc: string; content: string; hasLegRef: boolean }> = []
  ...
  for (const fn of allFootnotes) {
    ...
    } else if (c.includes('jfr') || c.includes('direktiv') || c.includes('rådets')) {
  ...
  console.log(`1. Legislative refs (prop/bet/rskr): ${categories.legislativeRefs.length}`)
  categories.legislativeRefs.slice(0, 3).forEach(c => console.log(`   "${c.substring(0, 80)}..."`))

  console.log(`\n2. "Senaste lydelse" (previous version refs): ${categories.senasteLydelse.length}`)
  categories.senasteLydelse.slice(0, 3).forEach(c => console.log(`   "${c.substring(0, 80)}..."`))

  console.log(`\n3. "Omtryckt" (reprinted notices): ${categories.forordningOmtryckt.length}`)
  categories.forordningOmtryckt.slice(0, 3).forEach(c => console.log(`   "${c.substring(0, 80)}..."`))

  console.log(`\n4. EU directive refs (Jfr direktiv): ${categories.jfrDirectiv.length}`)
  categories.jfrDirectiv.slice(0, 3).forEach(c => console.log(`   "${c.substring(0, 80)}..."`))

  console.log(`\n5. Other: ${categories.other.length}`)
  categories.other.slice(0, 5).forEach(c => console.log(`   "${c.substring(0, 100)}"`))
}
