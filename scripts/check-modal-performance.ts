...
async function checkModalPerformance() {
  ...
  try {
    ...
    console.log('   Set/Get test:', result === 'test-value' ? '✅ Working' : `❌ Failed (got: ${result})`)
  } catch (error) {
  ...
  if (cachedDoc) {
    ...
    const parsed = typeof cachedDoc === 'string' ? JSON.parse(cachedDoc) : cachedDoc
  ...
  for (const item of listItems.slice(0, 3)) {
    ...
    console.log(`   Item ${item.id.substring(0, 8)}...: ${cached ? '✅ Cached' : '❌ Not cached'}`)
  }
  
  // 6. Check database connection
  console.log('\n6️⃣ Testing database query performance:')
  const queryStart = Date.now()
  const testQuery = await prisma.legalDocument.findFirst({
    where: { document_number: 'SFS 1977:1160' },
    select: { id: true }
  })
  const queryDuration = Date.now() - queryStart
  console.log(`   Simple query took: ${queryDuration}ms ${queryDuration > 100 ? '⚠️ SLOW' : '✅'}`)
