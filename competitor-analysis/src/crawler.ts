...
class NotisumCrawler {
  ...
  async init() {
    ...
    ;[CONFIG.outputDir, CONFIG.screenshotsDir, CONFIG.reportsDir].forEach(
      (dir) => {
  ...
  async login() {
    ...
    try {
      ...
      console.log(
        '⚠️  The crawler will automatically detect when you are logged in'
      )
      ...
      try {
        ...
        console.log(
          '⚠️  No navigation detected, checking if we are already logged in...'
        )

        // Maybe we're already on the dashboard, check for typical elements
        try {
          await this.page.waitForSelector(
            'nav, .navigation, header, .dashboard, .main-content',
            { timeout: 5000 }
          )
  ...
  async analyzePage(pageName: string): Promise<PageAnalysis> {
    ...
    const forms = await this.page.$$eval('form', (elements) =>
      elements.map(
        (el) =>
          el.getAttribute('name') || el.getAttribute('id') || 'unnamed-form'
      )
    )

    const tables = await this.page.$$eval('table', (elements) =>
      elements.map((el, i) => {
        const headers = Array.from(el.querySelectorAll('th')).map((th: any) =>
          th.textContent?.trim()
        )
    ...
    const navigation = await this.page.$$eval(
  ...
  async exploreNavigation() {
    ...
    for (const link of allNavLinks) {
      ...
      try {
        ...
        const pageName = this.sanitizeFilename(
          link.text || `page-${this.visitedUrls.size}`
        )
  ...
  async explorePageLinks(depth: number = 1, currentDepth: number = 0) {
    ...
    console.log(
      `  🔍 Found ${links.length} links on current page (depth: ${currentDepth + 1}/${depth})`
    )
    ...
    for (const link of links) {
      ...
      if (
        ...
        try {
          ...
          const pageName = this.sanitizeFilename(
            link.text || `subpage-${this.visitedUrls.size}`
          )
          const analysis = await this.analyzePage(pageName)
  ...
  categorizeSection(linkText: string, url: string): string {
    ...
    if (
      ...
      return 'dashboard'
    if (text.includes('dokument') || text.includes('document'))
      return 'documents'
    if (
      text.includes('lag') ||
      urlLower.includes('law') ||
      urlLower.includes('lag')
    )
      return 'laws'
  ...
  async generateFeaturesSummary() {
    ...
    Array.from(allFeatures)
      .sort()
      .forEach((feature) => {
        const count = this.pageAnalyses.filter((a) =>
