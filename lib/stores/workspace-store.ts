...
interface WorkspaceState {
  ...
  // Actions
  setCurrentWorkspace: (_workspace: Workspace, _role: WorkspaceRole) => void
  setMembers: (_members: WorkspaceMember[]) => void
  setLawLists: (_lists: LawList[]) => void
  setListItems: (_listId: string, _items: LawListItem[]) => void
  setUserWorkspaces: (_workspaces: Workspace[]) => void
  updateUserPreferences: (
    _prefs: Partial<WorkspaceState['userPreferences']>
  ) => void

  // Cache management
  shouldRefetchMembers: () => boolean
  shouldRefetchLists: () => boolean
  shouldRefetchListItems: (_listId: string) => boolean
  clearWorkspaceCache: () => void

  // Optimistic updates
  addLawList: (_list: LawList) => void
  updateLawList: (_listId: string, _updates: Partial<LawList>) => void
  deleteLawList: (_listId: string) => void
  addListItem: (_listId: string, _item: LawListItem) => void
  removeListItem: (_listId: string, _itemId: string) => void

  // Loading state setters
  setLoadingWorkspace: (_loading: boolean) => void
  setLoadingMembers: (_loading: boolean) => void
  setLoadingLists: (_loading: boolean) => void
}
...
export const useWorkspaceStore = create<WorkspaceState>()(
  persist(
    immer((set, get) => ({
      ...
      updateLawList: (listId, updates) =>
        set((state) => {
          const item = state.lawLists.find((l: LawList) => l.id === listId)
          if (item) {
            Object.assign(item, updates)
          }
        }),

      deleteLawList: (listId) =>
        set((state) => {
          state.lawLists = state.lawLists.filter(
            (l: LawList) => l.id !== listId
          )
...
export const useLawLists = () => useWorkspaceStore((state) => state.lawLists)
