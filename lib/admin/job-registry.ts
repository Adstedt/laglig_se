...
export const JOB_REGISTRY: CronJobDefinition[] = [
  ...
  },
  {
    ...
    description:
      'Upptäcker nya SFS-ändringar från svenskforfattningssamling.se (endast upptäckt — bearbetning sker i process-sfs-amendments)',
    schedule: '0 10,13 * * *',
    scheduleHuman: 'Dagligen kl. 10:00 + 13:00 UTC',
    endpoint: '/api/cron/discover-sfs-amendments',
    authHeader: 'CRON_SECRET',
    instrumented: true,
  },
  {
    name: 'process-sfs-amendments',
    displayName: 'SFS-ändringsbearbetning',
    description:
      'Bearbetar väntande SFS-ändringar (PDF → AI-tolkning → paragrafändringar + händelser)',
    schedule: '20 * * * *',
    scheduleHuman: 'Varje timme vid :20',
    endpoint: '/api/cron/process-sfs-amendments',
    authHeader: 'CRON_SECRET',
    instrumented: true,
  },
  {
    name: 'sfs-pipeline-health',
    displayName: 'SFS-pipelinehälsa',
    description:
      'Vakthund: städar zombiekörningar, jämför officiella registret mot databasen och larmar via e-post vid problem',
    schedule: '30 7 * * *',
    scheduleHuman: 'Dagligen kl. 07:30 UTC',
    endpoint: '/api/cron/sfs-pipeline-health',
    authHeader: 'CRON_SECRET',
    instrumented: true,
  },
  {
