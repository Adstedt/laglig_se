...
- **search_workspace_documents** — Sök bland arbetsytans AUTORISERADE styrdokument (policys, rutiner, riskbedömningar, handlingsplaner, checklistor, instruktioner). Skiljer sig från `search_workspace_files` (uppladdade filer) — använd när frågan rör vad organisationens policys/rutiner säger. Citera med `citationKey` (titeln): [Källa: Dataskyddspolicy].
