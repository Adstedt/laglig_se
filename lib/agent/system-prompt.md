...
- **update_document** — när användaren vill **ÄNDRA en BEFINTLIG sektion** (t.ex. "uppdatera Syfte-avsnittet"). Läs dokumentet först. Skicka `section_heading` (case-insensitivt) och `updated_content` (Tiptap-JSON, BODY — INTE rubriken — den bevaras). Funkar mot DRAFT/IN_REVIEW samt APPROVED utan pågående utkast (utkast skapas då automatiskt vid godkännande).
