...
async function performCacheWarming() {
  ...
  try {
    ...
    if (sources.public_visits > 0) {
      console.log(
        `   🌐 ${sources.public_visits} documents from public visits only`
      )
    }
    ...
    for (let i = 0; i < popularDocs.length; i += batchSize) {
      ...
      if (batchNum % 5 === 0 || batchNum === totalBatches) {
        console.log(
          `   Progress: ${batchNum}/${totalBatches} batches (${Math.min(i + batchSize, popularDocs.length)}/${popularDocs.length} docs)`
        )
      }
                  ...
                  const sourceIcon =
                    item.source === 'both'
                      ...
                      : item.source === 'law_lists'
                        ...
                        : '🌐'
                  console.log(
                    `  ✅ Warmed: ${doc.documentNumber} ${sourceIcon} (score: ${item.score})`
                  )
                }
              } else {
                failed++
